# .airules/ 工作区 — change(OpenSpec) + 会话沉淀 + skill 提炼

## 目标（用户确认）

给用户项目接入 `.airules/` 工作区，三块能力：
1. **change 工作流**：装官方 OpenSpec CLI，用 stores 把工作目录落在 `.airules/openspec/`。
2. **会话沉淀**：自建 `session-capture` skill，写 `.airules/sessions/`。
3. **skill 提炼**：自建 `skill-distill` skill，扫 sessions/changes → `.airules/skills-candidates/` 待用户审核。

## 已确认决策

- change 用官方 OpenSpec CLI（不自建、不 fork）。
- 收进 `.airules/openspec/`（接受多一层 openspec/、命令文件落宿主目录、软门禁、全局依赖）。
...
1. `store setup --path` 是否接受相对路径 `.airules`（稳妥用 `"$PWD/.airules"` 绝对路径）。
...
- 新增 `skills/init-project/scripts/init-openspec.mjs`：
  ...
  - 用项目目录名派生 store id，跑 `openspec store setup <id> --path "$PWD/.airules"`（实测后定确切语法）。
  - 幂等：已注册则跳过（捕获 "already registered" 不报错）。
- SKILL.md 流程图加一环：codegraph init 后 → init-openspec（建 `.airules/openspec/` store）。
- 交付检查表加一行：OpenSpec store → `.airules/openspec/changes/` 可用，命令缺失报 MISSING。
...
- `skills/session-capture/SKILL.md`（无 description）：
  ...
  - 输出：`.airules/sessions/<date>-<topic>.md`，结构化记录本次会话的关键决策/踩坑/约定/可复用模式。
...
- `skills/skill-distill/SKILL.md`（无 description）：
  ...
  - 流程：扫 `.airules/sessions/` + `.airules/openspec/changes/`，识别可复用为 skill 的模式，生成 candidate 草稿到 `.airules/skills-candidates/<name>/SKILL.md`。
...
```
rules/ 这层暂不动（最小化 init-project 当前不建 .airules/rules/）。
...
- `node skills/init-project/scripts/init-openspec.mjs <tmp>` 手测 store 建在 `.airules/openspec/`。
...
- OpenSpec stores beta 三点需实测（见上）；若 `--path` 不吃相对路径或 init 顺序有坑，回退到"openspec/ 放项目根，与 .airules/ 并存"（选项B）并知会用户。
