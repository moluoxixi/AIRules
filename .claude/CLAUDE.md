# Moluoxixi Claude 全局基线

## 宿主入口

当 Claude / Claude Code 通过这个仓库安装后，把下面这些路径视为当前生效入口：

- `~/.claude/skills/` 是 Claude 读取的技能入口
- `~/.moluoxixi/skills/` 是被投影到该入口的源技能树
- `~/.claude/agents/` 是可选的辅助 agents 入口
- `~/.claude/CLAUDE.md` 是 Claude 侧的全局基线文件
...
这些第一方 skills 都位于 `~/.claude/skills/` 下，与 vendor skills 并列存在。
...
- 从 `~/.claude/skills/` 这个入口出发，而不是依赖全局 rules
...
- 确认 `~/.claude/skills/` 真的指向 `~/.moluoxixi/skills/`
- 确认需要的第一方 skills 已经存在于 Claude 的技能入口
- 如果 agents 存在，确认 `~/.claude/agents/` 已正确投影
