...
开发链路的关键环节默认走子代理隔离上下文，主代理只做拆分、协调、整合与复核。下表给出每个环节的委派形态与判据；命中「主代理直接做」的小规模例外时，必须在交付中说明为何不委派。`skill` 决定知识内容和方法论，`subagent` 决定上下文隔离、并行和反自评边界；不得因为“角色特点不同”就自动拆 agent。表中的具名 agent 按当前 AIRules 第一方 `agents/` 映射；宿主不支持同名 agent 时，按同等职责与隔离边界选择可用子代理。
...
- debugger 委派契约（bugfix 链前置）：复杂/跨栈/需交接的 bug，起独立上下文的 debugger 子代理调查根因——委派时自包含提供失败现象、复现步骤、相关文件与约束边界；debugger 只读诊断、复现与立假设，可落盘 `docs/diagnosis/<bug>.md`，**不改生产代码**（修复由 coder 按回传的建议修复点执行）；回传「根因 + 证据 + 建议修复点 + 回归测试设计」，是自述而非已验证事实，主代理须用可验证句柄复核后再驱动修复。单点已定位的小 bug 主代理直接修，不派 debugger。
- 委派打分口径（拆 agent vs 拆知识内容）：「角色有不同特点」只决定是否需独立**知识内容（skill）**，不直接决定是否需独立 **agent**；是否值得独立 agent 取容器级三理由之一——①隔离收益（产生大量主代理无需保留的中间 token）②可并行（不重叠文件树/文档树可同时跑）③独立性要求（写的人必须≠评的人）。如 debugger 因根因调查隔离收益高而独立、reviewer 因防自评而独立；test 编写仅「特点不同」但无容器级理由，故并入 coder 而不独立成 agent。
