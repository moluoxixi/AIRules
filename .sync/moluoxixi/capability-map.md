# Trellis Capability Map

Reviewed package baseline: upstream revision `bd454938dc406e2f692a07c3f3888e5375ff674d`.
The latest non-merge commit reconciliation from `v0.6.15` through the pinned post-release revision is recorded in [history/reconciliation-bd454938.json](history/reconciliation-bd454938.json); it links to the earlier `v0.6.7` through `v0.6.15` record.
Behaviors that future AI reviews must preserve are recorded in [preservation-contracts.json](preservation-contracts.json), including local extensions and explicit upstream parity contracts.
...
| `upgrade` | Global `airules install moluoxixi`; installation first updates a clean AIRules checkout, then projects use the installed role version; SessionStart and text context compare the installed role version with the project version and direct stale projects to the current `init-project` skill; project runtime never installs an npm CLI |
...
| Global setup and MCP | Role-owned CodeGraph install/setup plus JSON/TOML MCP projection for supported hosts; installed together with the selected role |
...
The initializer reads the upstream package's `src/templates` tree as its sole template source, then applies the hashed Moluoxixi overlay manifest and host projection rules. See [asset-layout.md](../../roles/moluoxixi/skills/init-project/references/asset-layout.md) for the source and output ownership contract.
