...
`distill-candidates` 从 `.airules/sessions/` 与 `.airules/changes/` 提炼出的 **declarative（是什么、为什么）** 事实落在这里，每条候选一个文件 `<slug>.md`。
...
distill-candidates 写入 (review_status: pending)
  ...
  → approved 的记忆候选：交 remember 转正写入 .airules/memory/ 并登记 MEMORY.md
  → 转正后删除此处候选文件
```
...
- 候选**永不自动生效、永不被 recall-memory 召回**（recall 只读 `.airules/memory/`）。
