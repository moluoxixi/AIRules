...
- 来源文档包含需求、接口、组件、测试、架构中的多类信息时，必须拆分到对应目录，不得把混合内容原样复制到单个目标文档。
- 转换目标必须使用标准位置：PRD 写入 `docs/prds/`，接口写入 `docs/api/`，组件写入 `docs/components/`，测试写入 `docs/test/`，架构写入 `docs/architecture/`。
