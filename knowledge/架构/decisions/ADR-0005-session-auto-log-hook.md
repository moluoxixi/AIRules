...
> 2026-07-04 更新：development runtime 回路 hook / ledger 链路已撤下；本 ADR 当前只约束 common `session-log.mjs` Stop hook。
...
- **中性源**：`hooks/session-log.mjs`（纯 Node、无依赖）。读 stdin JSON，在 `<cwd>/knowledge/sessions/auto/<日期>.log` 追加一行索引（时间戳 + 事件 + session + transcript 路径 + cwd）。
...
- **记录粒度**：transcript 路径引用，不复制正文、不回显敏感值（与 `session-capture` 写入边界一致）；各项目各自落 `knowledge/sessions/auto/`，首次建目录写 `.gitignore` 忽略 `*.log`。
