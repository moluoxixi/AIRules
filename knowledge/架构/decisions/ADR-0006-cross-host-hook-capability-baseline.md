...
> 2026-07-04 更新：本 ADR 的能力基线仍作事实参考；development runtime 回路 hook / ledger 链路已按角色收敛方向撤下，当前 AIRules 仅分发 common Stop session-log hook。
...
- **本 ADR 只确立能力基线与历史边界，不实现任何阻断 hook**。当前 AIRules 不再分发 development runtime 回路 hook / ledger 链路；若未来重新引入运行时阻断，需重新立项并复核宿主能力、客观信号边界与角色分发范围。
- 修正 ADR-0005 替代方案段的作废论据（已留痕，结论不变）。
- `host-hook-mapping.md` 的"永不阻断"红线当前适用于 Stop session-log hook。
- `constants/hosts.ts` 的 `HookProjection | HookProjection[]` 结构仍保留多事件表达能力，但当前已登记宿主仅投影 common Stop session-log hook。
