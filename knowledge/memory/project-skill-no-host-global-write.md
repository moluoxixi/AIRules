...
**How to apply**：写 `init-project` 类或任何项目级 skill 时，安装/写入路径只落项目内（项目根、`knowledge/`、`openspec/` 等），不碰宿主 home。此约束有客观信号兜底——[check-rules-consistency.ts](../../scripts/check-rules-consistency.ts) check #9 扫 `skills/` 下 SKILL.md 与安装脚本（扩展名 sh、bash、ps1、py、ts、js、mjs、cjs），命中宿主全局目录引用即报错。即「不只靠 prose、有脚本拦截」。
