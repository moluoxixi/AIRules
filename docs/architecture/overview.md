...
| Knowledge Search | 通过登记文件系统来源或临时本地来源查找证据，并输出 `PASS`、`MISSING evidence`、`MISSING conflict`、`FAIL` 或 `NOT RUN`。 | `airules.knowledge.json`、filesystem sources | 用户回答、docs skills、待审学习候选文档 | project-maintainer |
...
5. 任务经验需要沉淀时，先向用户说明候选内容和写入位置；用户确认后再写入 `PENDING_REVIEW` 待审文档。
