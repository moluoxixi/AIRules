...
| Knowledge Source Registry | 登记可检索项目知识源，约束 include/exclude、owner、trust 和 Khoj collection。 | `airules.knowledge.json` | `knowledge-search`、docs skills、证据报告 | project-maintainer |
| Knowledge Search | 通过登记源、Khoj collection 或临时本地来源查找证据，并输出 `PASS`、`MISSING evidence`、`MISSING conflict`、`FAIL` 或 `NOT RUN`。 | `airules.knowledge.json`、Khoj、filesystem sources | 用户回答、docs skills、learning-capture 候选 | project-maintainer |
...
- 文档输出层只能在明确触发的 docs skill 中写入正式文档；Khoj 和 MemPalace 均不得直接写正式 docs。
- 记忆层只沉淀代理经验候选，不替代项目知识源或标准文档输出。

## 数据流

1. `init-project` 读取目标项目，生成或保留 `airules.knowledge.json`。
2. `knowledge-search` 校验注册表，按 source type 查询文件系统或 Khoj collection。
...
- Khoj 缺少服务地址、凭证或 collection 时必须报告 `MISSING khoj config`，不得降级为本地成功。
...
- Khoj 当前只作为可选外部检索层；AIRules 不安装、托管或假设 Khoj 运行时。

## 待确认

- MISSING：Khoj 官方 API endpoint、认证方式、collection/filter 语义和失败响应格式需要在实现查询适配器前重新核实。
