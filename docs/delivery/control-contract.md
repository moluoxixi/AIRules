...
所有生成、修改、删除动作必须先判定级别，判定边界模糊或缺少判定事实时按 L2 处理：

| 级别 | 含义 | 确认要求 |
|---|---|---|
| L0 | 既有口径、契约或模式内补充，不改变任何对外事实 | 可直接执行，交付说明判定依据和验证结果 |
| L1 | 既有边界内新增或修改，不触及对外口径、协议、边界、权限、状态机、数据一致性或安全边界 | 可直接执行，交付附判定依据和验证结果 |
| L2 | 触及需求口径、架构边界、公共协议、权限、状态机、数据一致性、安全边界、跨模块行为，或修改 rules、skills、初始化流程、默认分发配置 | 必须先完成澄清门禁并输出报告/计划，等待用户确认 |

该分级定义同时落在 `rules/AGENTS.md` 与 `skills/init-project/references/common/control.md`，由 `init-project` 注入下游项目 `AGENTS.md`，确保各宿主 agent 拿到同一套闸门。

## 澄清触发机制

| 触发条件 | 动作 | 失败语义 |
|---|---|---|
| 命中 L2，或目标/角色/边界/流程/字段/状态/验收/冲突/风险任一关键事实 `MISSING` 或存在歧义 | 先输出《澄清问题清单》或对应设计报告，苏格拉底式逐项暴露上述维度 | 澄清未闭环即定稿标记 `FAIL` |
| 未确认事实 | 保留为 `MISSING`，不得用推断、默认值或代码反推替代 | 用推断替代缺失事实标记 `FAIL` |

prd-docs、architecture-docs、test-docs、api-docs、components-docs 的苏格拉底澄清统一引用本机制，不得作为可跳过的装饰性说明。
