...
- `sync` refreshes selected remote assets, intersects the public host registry with the selected role's host ID allowlist, projects canonical skills, runs selected role setup commands, and projects selected role MCP declarations where a host has an MCP contract.
- `verify` checks AIRules-managed skill projections. Project-native agents, commands, hooks, plugins, extensions, and settings belong to project initializers and are left untouched by the common layer.
