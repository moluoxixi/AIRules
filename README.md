...
Replace `moluoxixi` with `matt` to install the Matt role. Later updates only require `airules install <role> --host all`; a clean source checkout is fast-forwarded before the role is installed and verified.
