...
- The canonical `.agents/skills`, `.claude/agents`, and root `AGENTS.md` are mapped to standard role skills, agents, and rules; the complete role path also retains the selected native multi-host adapters. The AIRules-owned `init-project` skill generates the project runtime and 18 platform integrations from migrated role assets. It does not install or invoke the Trellis npm CLI.
