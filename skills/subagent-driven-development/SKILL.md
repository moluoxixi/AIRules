...
**fix→复审循环上限**：同一 task 的 `fix 子代理 → task reviewer 复审` 循环不得超过 2 次。这一步发生在「实现」单阶段内部，不会触发主编排 `Test→Debug→Code` 的 `max_loop` 计数，故必须自带上限——否则一个 task 可在到达测试前无限 fix→复审空转。超限即标该 task 为 `BLOCKED`，记进度账本（含已尝试 fix 与残留发现），汇入主编排的升级路径交用户决策，不得让同一 task 无变化反复回炉。这与 debugger"同一思路失败两次换方向"的铁律对齐。
