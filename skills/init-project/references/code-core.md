...
- 测试代码用例ID 回溯：当存在测试设计（`.airules/tests/<feature>.md` 的用例矩阵，用例ID 形如 `TC-<feature>-<序号>`；历史项目也可能位于 `docs/test/<模块>.md`，格式相同）时，每条落地的测试用例（`it`/`test`/`@Test` 等）必须在其紧邻上方注释标注所实现的用例ID（如 `// TC-采购订单-005`，多用例用逗号分隔）；使测试设计与测试代码可机器双向反查（`consistency-check` 据此核对「用例 → 测试代码」）。无测试设计的临时探查测试不强制标注。不得为凑标注编写空断言或无意义测试。
