...
- 单模块测试设计写入 `docs/test/<模块>.md` 或既有模块测试文档。
- 跨模块端到端旅程（横跨两个或更多需求模块的完整用户旅程）写入 `docs/test/e2e/<旅程名>.md`，并在 `docs/test/index.md` 维护旅程清单：旅程名、涉及模块和状态。
- 当某模块参与跨模块旅程时，对应测试设计必须说明本模块负责的旅程步骤；无法确认时标记 `MISSING journey step`。
