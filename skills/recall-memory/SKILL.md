...
3. **status 过滤**：默认只召回 `status: active`；`superseded` 不进召回（除非用户显式要追溯历史决策）。引用前看 `created_at`，对年久且涉及具体文件/接口的记忆加倍复核时效。
4. **边界最低召回**：每次召回结果中至少包含一条相关的 `constraint`/`boundary`（若存在）——防止纯执行类记忆（教"如何做"）在高权重召回中持续覆盖应谨慎/拒绝的判断（安全边界侵蚀）。本项目无 boundary 记忆时跳过，不硬凑。
...
- **快照复核**：记忆是写入时刻的事实快照。若某条记忆命名了具体文件、函数、配置标志，引用前必须复核它在当前代码中仍然存在；已失效的记忆不据此下结论，并提示可能需要更新（交 `remember` 修订或标 `superseded`）。高置信度但已过时的记忆比低相关记忆危害更大——它会以高权重被召回并误导决策，故宁可漏召一条过时记忆，不可让 `superseded` 混入。
