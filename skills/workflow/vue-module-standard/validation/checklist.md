...
```bash
node skills/workflow/vue-module-standard/scripts/verify-rules.mjs
node skills/workflow/vue-module-standard/scripts/verify-rules.mjs module --root src/views/purchaseOrder
node skills/workflow/vue-module-standard/scripts/verify-rules.mjs hoist --target src/views/orderShared/utils --uses src/views/purchaseOrder/index.vue src/views/salesOrder/index.vue src/views/refundOrder/index.vue
```
...
5. 状态是否就近保留，没有无依据上浮到 store、provide/inject 或 composable？
...
9. 是否运行了与风险匹配的现有 lint、vue-tsc、test、build 或浏览器验证？
