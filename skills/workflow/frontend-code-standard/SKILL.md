...
description: 用于编写、修改或评审 Vue 3 或 React TypeScript 前端代码，强制执行分形架构、特性驱动目录、Headless 逻辑、路径别名、逐级上浮、类型拆分、统一导出、依赖流向和注释契约。
---
...
- 组件包结构：组件包或复杂组件目录必须包含 `README.md`、`index.ts` 或 `index.js`，以及 `src/`；`README.md` 必须描述组件用途、使用方式和接口契约。
