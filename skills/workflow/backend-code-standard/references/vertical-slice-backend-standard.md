...
业务模块或需要形成稳定公共 API 的功能集目录，必须提供一个 `index.ts` 文件作为对外入口。模块私有子目录不强制创建 `index.ts`；只有当目录需要被同级外部模块消费时，才建立明确的 public surface。

- 路径别名优先：跨模块引用或涉及多层向上查找（如 `../../`）时，必须优先使用项目配置的路径别名（如 `@/` 或 `~/`）。
- Deep Imports 零容忍：跨领域引用必须止步于目标模块的公共入口；模块内部文件之间可按就近原则引用私有文件，但不得把私有路径扩散给其他领域。
