...
const REACT_IMPLEMENTATION_FILENAMES = ['index.tsx', 'index.jsx']
...
function assertCodeDirectoryEntries(directory, root, depth = 0) {
  ...
  for (const entry of entries) {
    ...
    const childImplementationEntries = findExistingFiles(childDirectory, REACT_IMPLEMENTATION_FILENAMES)
...
function assertComponentPackage(root) {
  ...
  const publicEntry = assertSingleExistingFile(componentRoot, PUBLIC_ENTRY_FILENAMES, 'React 复杂组件包根目录公共入口')
  const rootImplementationEntries = findExistingFiles(componentRoot, REACT_IMPLEMENTATION_FILENAMES)

  if (!fs.existsSync(readmePath))
    throw new Error('React 复杂组件包根目录缺少 README.md')

  if (!fs.existsSync(srcPath) || !fs.statSync(srcPath).isDirectory())
    throw new Error('React 复杂组件包根目录缺少 src/ 实现目录')

  if (rootImplementationEntries.length > 0)
    throw new Error(`React 复杂组件包根目录不得放置实现入口：${rootImplementationEntries.join('、')}`)

  const srcImplementationEntry = assertSingleExistingFile(srcPath, REACT_IMPLEMENTATION_FILENAMES, 'React 复杂组件包 src/ 实现入口')
  const readme = fs.readFileSync(readmePath, 'utf8').trim()

  if (readme.length === 0)
    throw new Error('React 复杂组件 README.md 不得为空，必须描述组件如何使用')

  if (!/(使用|用法|Usage|Props|Ref|Children|API)/.test(readme))
    throw new Error('React 复杂组件 README.md 必须包含使用方式或接口契约说明')

  assertCodeDirectoryEntries(componentRoot, componentRoot)

  printPass('react component package structure is valid', {
    componentRoot,
    entry: publicEntry,
...
function assertSimpleComponent(root) {
  ...
  if (!/^[A-Z][\w-]*\.(tsx|jsx)$/.test(filename))
    throw new Error('React 简单组件必须使用 ComponentName.tsx 或 ComponentName.jsx 文件')

  if (!fs.existsSync(componentPath) || !fs.statSync(componentPath).isFile())
    throw new Error('React 简单组件路径必须指向真实文件')

  printPass('react simple component structure is valid', {
    componentPath,
  })
}
...
示例:
  ...
  node scripts/verify-rules.mjs simple-component --root src/components/StatusBadge.tsx
...
function verifySelf() {
  ...
  assertContains(skill, /React 18/, 'SKILL.md 必须声明 React 18 版本要求')
  assertContains(skill, /React 19/, 'SKILL.md 必须声明 React 19 版本要求')
  assertContains(skill, /forwardRef/, 'SKILL.md 必须覆盖 forwardRef')
  assertContains(skill, /useImperativeHandle/, 'SKILL.md 必须覆盖 useImperativeHandle')
  assertContains(skill, /Error Boundary/, 'SKILL.md 必须覆盖 Error Boundary')
  assertContains(skill, /Suspense/, 'SKILL.md 必须覆盖 Suspense')
  assertContains(skill, /useMemo/, 'SKILL.md 必须覆盖 useMemo')
  assertContains(skill, /useCallback/, 'SKILL.md 必须覆盖 useCallback')
  assertContains(skill, /memo/, 'SKILL.md 必须覆盖 memo')
  assertContains(skill, /Context/, 'SKILL.md 必须覆盖 Context')
  assertContains(skill, /useEffect/, 'SKILL.md 必须覆盖 useEffect')
  assertContains(skill, /受控/, 'SKILL.md 必须覆盖受控组件模式')
  ...
  assertContains(skill, /19\+.*ref.*prop/s, 'SKILL.md 必须说明 React 19 的 ref as prop')
  assertContains(skill, /Context\.Provider/, 'SKILL.md 必须覆盖 Context.Provider 用法')

  assertContains(componentExample, /useImperativeHandle/, '组件示例必须覆盖 useImperativeHandle')
  assertContains(componentExample, /forwardRef/, '组件示例必须覆盖 forwardRef')
  assertContains(componentExample, /Error Boundary|ErrorBoundary/, '组件示例必须覆盖 Error Boundary')
  assertContains(componentExample, /createContext|useContext/, '组件示例必须覆盖 Context')
  ...
  assertContains(checklist, /React 19/, '校验文件必须覆盖 React 版本检查')

  assertContains(checklist, /forwardRef/, '校验文件必须覆盖 forwardRef 检查')
  assertContains(checklist, /Context\.Provider/, '校验文件必须覆盖 Context.Provider 检查')

  printPass('react-component-standard self rules are valid')
}
