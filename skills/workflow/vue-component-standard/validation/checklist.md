...
```bash
node skills/workflow/vue-component-standard/scripts/verify-rules.mjs
node skills/workflow/vue-component-standard/scripts/verify-rules.mjs simple-component --root src/components/StatusBadge.vue
node skills/workflow/vue-component-standard/scripts/verify-rules.mjs component --root src/components/DataTable
```
...
4. Vue 版本相关 API 是否正确使用？
   - `defineModel` 需要 Vue 3.4+，低版本需回退到 `modelValue` / `update:modelValue`。
   - `useTemplateRef` 需要 Vue 3.5+，低版本需回退到 `ref()` + 模板 `ref="xxx"`。
