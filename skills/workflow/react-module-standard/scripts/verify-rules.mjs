...
function assertModule(root) {
  ...
  const implementationEntry = assertSingleExistingFile(moduleRoot, MODULE_IMPLEMENTATION_FILENAMES, 'React 模块根目录实现入口')
  ...
  printPass('react module structure is valid', {
    moduleRoot,
    implementationEntry,
  })
}
...
function verifySelf() {
  ...
  assertContains(skill, /Context/, 'SKILL.md 必须覆盖 Context 规则')
  ...
  assertContains(businessModuleExample, /Context/, '模块示例必须覆盖 Context')
  assertContains(businessModuleExample, /useContext/, '模块示例必须覆盖 useContext')
...
function verifyHoist(args) {
  ...
  printPass('react module hoist target stays under nearest common ancestor', {
