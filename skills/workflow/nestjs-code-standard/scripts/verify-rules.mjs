...
function verifySelf() {
  ...
  // Keep security, documentation, CQRS, and domain exception guidance from regressing silently.
  assertContains(skill, /ClassSerializerInterceptor/, 'SKILL.md 必须覆盖响应序列化拦截器')
  assertContains(skill, /@Exclude\(\)/, 'SKILL.md 必须覆盖响应字段排除策略')
  assertContains(skill, /@Expose\(\)/, 'SKILL.md 必须覆盖响应字段显式暴露策略')
  assertContains(skill, /@nestjs\/cqrs/, 'SKILL.md 必须覆盖官方 CQRS 生态')
  assertContains(skill, /CommandHandler/, 'SKILL.md 必须覆盖 CommandHandler')
  assertContains(skill, /QueryHandler/, 'SKILL.md 必须覆盖 QueryHandler')
  assertContains(skill, /@Req\(\)/, 'SKILL.md 必须约束 @Req 使用')
  assertContains(skill, /@Res\(\)/, 'SKILL.md 必须约束 @Res 使用')
  assertContains(skill, /@Headers\(\)/, 'SKILL.md 必须约束 @Headers 使用')
  assertContains(skill, /@CurrentUser\(\)/, 'SKILL.md 必须建议自定义参数装饰器')
  assertContains(skill, /Domain Service 使用 `@Injectable\(\)`/, 'SKILL.md 必须覆盖 Domain Service DI 折中')
  assertContains(skill, /@ApiProperty\(\)/, 'SKILL.md 必须覆盖 Swagger DTO 属性契约')
  assertContains(skill, /@ApiOperation\(\)/, 'SKILL.md 必须覆盖 Swagger Controller 语义')
  assertContains(skill, /BaseDomainException/, 'SKILL.md 必须覆盖领域异常统一基类')
  ...
  assertContains(skill, /abstract class/, 'SKILL.md 必须覆盖 abstract class DI 契约')
  assertContains(skill, /Inject\('TOKEN'\)/, 'SKILL.md 必须覆盖显式 token 注入')
  assertContains(skill, /Exception Filter|@Catch\(\)/, 'SKILL.md 必须覆盖异常映射 Filter')
  assertContains(skill, /@nestjs\/config/, 'SKILL.md 必须覆盖配置校验')
  assertContains(skill, /Test\.createTestingModule/, 'SKILL.md 必须覆盖 Nest 测试模块')
  assertContains(skill, /overrideProvider/, 'SKILL.md 必须覆盖 provider override 测试装配')
  ...
  assertContains(skill, /src\/modules\/orders\//, 'SKILL.md 必须包含 NestJS 模块结构示例')
  assertContains(skill, /orders\.module\.ts/, 'SKILL.md 必须包含 module 文件示例')
  assertContains(skill, /@Module/, 'SKILL.md 必须覆盖 @Module')
  assertContains(skill, /ValidationPipe/, 'SKILL.md 必须覆盖 ValidationPipe')
  assertContains(skill, /class-validator/, 'SKILL.md 必须覆盖 class-validator')
