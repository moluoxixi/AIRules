...
description: 用于开发任务完成后刷新 docs/AI项目知识；若知识库不存在则从仓库事实创建基线知识库，若已存在则按 commit 增量更新索引、映射，并为需求、设计、接口、组件和运维知识输出待确认候选。
---
...
本 Skill 处理任务完成后的项目知识沉淀。它不记录任务状态，不定义宿主触发机制；调用方触发后，本 Skill 必须自行判断是创建知识库基线还是按 commit 增量刷新。

若本 Skill 没有被触发，交付报告必须把知识刷新标为 `NOT RUN`，并说明缺少触发结果。
...
- 开发任务完成且必要验证已结束。
- 用户明确要求“初始化知识库”“生成项目知识”“更新知识库”“刷新项目知识”“同步文档”。
