...
- 拆分和并行判断：见下方“任务拆分”。
- 项目知识读取工作流：`project-knowledge-read-workflow`。开发任务、需求分析或代码审查开始前，若项目存在 `docs/AI项目知识/项目概览.md`，使用它按需读取需求文档、接口文档、组件库、测试规范、项目规范和部署运维索引；若知识库不存在，只报告 `MISSING`，不创建知识库。
...
项目知识库以领域 `索引.md` 作为入口：需求文档、接口文档、组件库、测试规范和项目规范是只读导入目录，可由 Git 子模块提供；AI 只能读取和检查存在性，不得创建、改写或补全；部署运维可沉淀确认后的项目内规则。任务依赖文档正文但当前上下文缺少入口或内容时，必须报告 `MISSING`，不得用源码推测补全。
