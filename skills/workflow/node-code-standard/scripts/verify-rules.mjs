...
function verifySelf() {
  ...
  assertContains(skill, /Core Standards/, 'SKILL.md 必须声明 Core Standards 定位')
  ...
  assertContains(skill, /Review Protocol/, 'SKILL.md 必须覆盖 Review Protocol')
  assertContains(skill, /评审结论/, 'SKILL.md 必须包含评审结论')
  assertContains(skill, /检查范围/, 'SKILL.md 必须包含检查范围')
  assertContains(skill, /问题清单/, 'SKILL.md 必须包含问题清单')
  ...
  assertContains(skill, /## 8\. 自校验脚本/, 'SKILL.md 必须包含自校验脚本 section')
  assertContains(skill, /node scripts\/verify-rules\.mjs/, 'SKILL.md 必须声明自校验命令')
