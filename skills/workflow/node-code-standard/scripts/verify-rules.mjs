...
function verifySelf() {
  ...
  assertContains(skill, /业务语义解绑/, 'SKILL.md 必须声明业务语义类型不强制从 Transport Schema 推导')
  assertContains(skill, /失败显性化/, 'SKILL.md 必须覆盖失败显性化')
  assertContains(skill, /process\.env/, 'SKILL.md 必须覆盖环境配置校验')
  assertContains(skill, /DTO、领域对象、持久化模型分离/, 'SKILL.md 必须覆盖 DTO 与持久化模型隔离')
  assertContains(skill, /UseCase 级事务封装/, 'SKILL.md 必须覆盖 UseCase 级事务边界')
  ...
  assertContains(skill, /Sequelize\.Transaction/, 'SKILL.md 必须禁止 Sequelize 事务句柄泄漏')
  ...
  assertContains(skill, /Async Error Propagation/, 'SKILL.md 必须覆盖异步错误传播')
  ...
  assertContains(skill, /CORS allowlist/, 'SKILL.md 必须覆盖严格 CORS 策略')
  assertContains(skill, /Pino、Winston|Pino\/Winston|Pino|Winston/, 'SKILL.md 必须覆盖结构化日志库')
  assertContains(skill, /纯 JSON/, 'SKILL.md 必须覆盖 JSON 日志输出')
  ...
  assertContains(skill, /dead-letter queue/, 'SKILL.md 必须覆盖消费者失败语义')
  ...
  assertContains(skill, /`N\/A`/, 'SKILL.md 必须约束 N/A 状态')
  assertContains(skill, /## 示例/, 'SKILL.md 必须包含示例 section')
  assertContains(skill, /## 13\. 检查清单/, 'SKILL.md 必须包含检查清单 section')
  assertContains(skill, /## 14\. 自校验脚本/, 'SKILL.md 必须包含自校验脚本 section')
  ...
  assertContains(skill, /Application（编排层）/, 'SKILL.md 必须覆盖 application 边界')
  assertContains(skill, /Repository 只负责持久化访问和映射/, 'SKILL.md 必须覆盖 repository 边界')
  assertContains(skill, /目标分类：`application-module`/, 'SKILL.md 必须包含评审示例目标分类')
  assertContains(skill, /评审结论：`FAIL`/, 'SKILL.md 必须包含评审示例结论')
  assertContains(skill, /事务要求/, 'SKILL.md 必须覆盖事务要求')
  assertContains(skill, /\[HOIST_WARNING\]/, 'SKILL.md 必须覆盖共享边界风险扫描')
  assertContains(skill, /## 10\. 测试边界/, 'SKILL.md 必须包含测试边界 section')
  assertContains(skill, /纯单元测试/, 'SKILL.md 必须覆盖 domain 和 application 单元测试边界')
  assertContains(skill, /Testcontainers/, 'SKILL.md 必须覆盖真实数据库集成测试')
  assertContains(skill, /Supertest/, 'SKILL.md 必须覆盖 Express 路由集成测试')
  assertContains(skill, /fastify\.inject\(\)/, 'SKILL.md 必须覆盖 Fastify inject 测试入口')
  assertContains(skill, /Transport 集成测试是否覆盖异步错误进入统一 Error Boundary/, 'SKILL.md 必须在检查清单覆盖异步错误验证')
  assertContains(skill, /是否没有主线程大对象同步处理、CPU 密集同步任务或 ReDoS 风险/, 'SKILL.md 必须在检查清单覆盖主线程阻塞风险')
  assertContains(skill, /是否处理 `SIGINT\/SIGTERM` 并实现 Graceful Shutdown/, 'SKILL.md 必须在检查清单覆盖优雅退出')
