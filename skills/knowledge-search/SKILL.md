...
- 标准 docs 是可审计输出层；只有用户明确要求生成或更新文档时，才调用对应文档 skill（需求事实落盘用 `prd-docs`，需求方法论辅助用 pm-skills `deliver-prd`/`deliver-user-stories`，视觉侧用 `design-docs`，Spec 契约留痕用 OpenSpec `propose`）写入。
