...
本文档定义了 skill-creator 使用的 JSON schemas。

---

## evals.json

为 skill 定义评估用例。位于 skill 目录内的 `evals/evals.json`。
...
评分器的输出。位于 `<run-dir>/grading.json`。
...
- `eval_feedback`: （可选）对评估用例的改进建议，仅在评分器识别到值得提出的问题时出现
...
**如何捕获：** 当子 agent 任务完成时，任务通知包含 `total_tokens` 和 `duration_ms`。立即保存这些数据——它们不会持久化到其他地方，事后无法恢复。
...
- `metadata`: 关于基准测试运行的信息
  ...
  - `timestamp`: 基准测试运行的时间
  - `evals_run`: 评估用例名称或 ID 的列表
  - `runs_per_configuration`: 每个配置的运行次数（例如 3）
- `runs[]`: 单次运行结果
  - `eval_id`: 数字评估用例标识符
  - `eval_name`: 可读的评估用例名称（在查看器中用作章节标题）
...
**重要提示：** 查看器完全按照这些字段名读取数据。使用 `config` 而不是 `configuration`，或者将 `pass_rate` 放在运行的顶层而不是嵌套在 `result` 下，会导致查看器显示空值/零值。手动生成 benchmark.json 时务必参考此 schema。
