...
1. Run `{{PYTHON_COMMAND}} ./.trellis/scripts/knowledge.py status --json`. Stop when
...
2. Read [references/organization.md](references/organization.md),
   ...
   `{{PYTHON_COMMAND}} ./.trellis/scripts/knowledge.py sources --json` when
...
6. Run `{{PYTHON_COMMAND}} ./.trellis/scripts/knowledge.py acknowledge --batch <batch_id>`.
