...
最小化项目初始化：注入项目规则、建立 `CLAUDE.md` 链接、初始化 CodeGraph、运行 OpenSpec 项目初始化与宿主入口安装，确认 AIRules role sync 已通过 vendor sparse clone/projection 安装精选 BMAD skills，并按目标项目类型注入 OpenSpec schema：默认从上游克隆并安装 `superpowers-bridge`；前端项目直接复制维护的 `frontend-superpowers-bridge` schema 资产。
...
| OpenSpec schema + BMAD projected skills | `node "<init-project-skill>/scripts/spec-init.mjs" <project>` | 先通过 `openspec config path` 定位 OpenSpec 全局配置，并写入 `profile: custom`、`delivery: both`、全量 workflows（`propose, explore, new, continue, apply, ff, sync, archive, bulk-archive, verify, onboard`），确保 `/opsx:continue` 等全量 `/opsx:*` commands 与 skills 都会被官方 CLI 生成；随后按目标项目已有宿主目录运行 `openspec init <project> --tools <detected> --no-color` 安装 OpenSpec 官方入口；支持 `.claude`、`.codex`、`.cursor`、`.qoder`、`.trae`、`.opencode`，都不存在时默认 `qoder`。不运行 `bmad-method install --modules bmm`；BMAD 只走 AIRules role sync 的 vendor sparse clone/projection，脚本确认同一宿主 skills 目录下存在 `bmad-prd`、`bmad-create-epics-and-stories`、`bmad-generate-project-context`、`bmad-shard-doc`。随后按目标项目类型选择 schema：非前端项目从 `https://github.com/JiangWay/openspec-schemas.git` 克隆 `superpowers-bridge/` 并复制到 `openspec/schemas/superpowers-bridge/**`；前端项目不改写克隆内容，直接复制维护的 `frontend-superpowers-bridge` schema 资产到 `openspec/schemas/frontend-superpowers-bridge/**`；再从 `assets/knowledge/index.md` 复制缺失文件到 `knowledge/index.md`，把 `openspec/config.yaml` 设为 `schema: <selected-schema>`，并运行 `openspec schema validate <selected-schema>` 与 `openspec schemas` 确认项目级注册 | 幂等，已存在 schema/knowledge 文件不覆盖；`git`、`openspec` 缺失或 BMAD projected skills 缺失时标 `MISSING` 并失败；`openspec config path` 不返回路径、`package.json` 无法解析、维护的前端 schema 资产缺失或配置 JSON 无法解析时显式失败，不用 core profile、本地骨架、独立 frontend skill、运行时 schema 改写或 BMAD runtime installer 伪装完整初始化 |
