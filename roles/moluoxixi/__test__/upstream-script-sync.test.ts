...
function temporaryProject(platform = 'kilo', initialize = true) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'moluoxixi-v0615-scripts-'))
  roots.push(root)
  if (initialize) {
...
describe('trellis v0.6.15 project script adaptations', () => {
  it('supports task JSON output, metadata, base-branch resolution, and orphan rendering', () => {
    ...
    const sessionEnv = { ...process.env, MOLUOXIXI_CONTEXT_ID: 'script-sync' }
  ...
  it('skips only the configured per-turn breadcrumb and allows an empty keyword', () => {
    ...
    const openCodePlugin = fs.readFileSync(
      path.join(skillRoot, 'assets', 'hosts', 'opencode', 'plugins', 'inject-workflow-state.js'),
      'utf8',
    )
  ...
  it('keeps atomic-write cleanup and bounded polyrepo probes in the distributed source', () => {
    const scripts = path.join(skillRoot, 'assets', 'project', 'scripts', 'common')
    const io = fs.readFileSync(path.join(scripts, 'io.py'), 'utf8')
    expect(io).toContain('os.close(fd)')
    expect(io).not.toContain('except BaseException')

    const sessionContext = fs.readFileSync(path.join(scripts, 'session_context.py'), 'utf8')
  ...
  it('keeps the v0.6.15 Pi model, thinking, and native-session fixes', () => {
    const pi = fs.readFileSync(
      path.join(skillRoot, 'assets', 'hosts', 'pi', 'extensions', 'moluoxixi', 'index.ts.txt'),
      'utf8',
    )
  ...
  it('decodes hook JSON as UTF-8 and keeps generated-mechanism docs factual', () => {
    const core = path.join(skillRoot, 'assets', 'core')
    for (const name of ['inject-subagent-context.py', 'inject-shell-session-context.py']) {
      const hook = fs.readFileSync(path.join(core, 'hooks', name), 'utf8')
  ...
  it('uses the local Codex auto default without claiming a fork_turns limitation', () => {
    const hook = path.join(skillRoot, 'assets', 'core', 'hooks', 'inject-workflow-state.py')
    const script = [
