...
Skills and commands are textual entry points for user interaction with Moluoxixi. Different platforms use different names, but their core purpose is the same: tell the AI how to enter the Moluoxixi flow when the user expresses a certain intent.
...
Moluoxixi workflow skills usually share one semantic set: brainstorm, before-dev, check, update-spec, break-loop. Multi-file built-in skills such as `meta` use layered references.
...
They should not store task state; task state belongs in `.moluoxixi/tasks/` and `.moluoxixi/.runtime/`.
...
1. **Keep entry files short; references carry long content**. This matters especially for multi-file skills like `meta`.
