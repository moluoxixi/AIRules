...
Triggered from /moluoxixi:start when the user describes a development task, especially when:
...
```bash
TASK_DIR=$(python3 ./.moluoxixi/scripts/task.py create "brainstorm: <short goal>" --slug <auto>)
```
...
### Delegate to `moluoxixi-research` sub-agent (don't research inline)

For each research topic, **spawn a `moluoxixi-research` sub-agent via the Task tool** — don't do WebFetch / WebSearch / `gh api` inline in the main conversation.
...
✅ Good:
```
Main agent: Task(subagent_type="moluoxixi-research",
                 prompt="Research topic A; persist to research/topic-a.md")
          + Task(subagent_type="moluoxixi-research",
...
Each `moluoxixi-research` sub-agent should:
...
```bash
# Create child tasks
CHILD1=$(python3 ./.moluoxixi/scripts/task.py create "Child task 1" --slug child1 --parent "$TASK_DIR")
CHILD2=$(python3 ./.moluoxixi/scripts/task.py create "Child task 2" --slug child2 --parent "$TASK_DIR")

# Or link existing tasks
python3 ./.moluoxixi/scripts/task.py add-subtask "$TASK_DIR" "$CHILD_DIR"
```
...
| Command | When to Use |
|---------|-------------|
| `/moluoxixi:start` | Entry point that triggers brainstorm |
| `/moluoxixi:finish-work` | After implementation is complete |
| `/moluoxixi:update-spec` | If new patterns emerge during work |
