...
```
Development Flow:
  Learn something → /moluoxixi:update-spec → Knowledge captured
       ↑                                  ↓
  /moluoxixi:break-loop ←──────────────────── Future sessions benefit
  (deep bug analysis)
```

- `/moluoxixi:break-loop` - Analyzes bugs deeply, often reveals spec updates needed
- `/moluoxixi:update-spec` - Actually makes the updates
- `/moluoxixi:finish-work` - Reminds you to check if specs need updates
