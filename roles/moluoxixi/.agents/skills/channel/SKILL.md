---
name: channel
description: Use Moluoxixi channel for live multi-agent collaboration, spawned workers, cross-agent review, progress inspection, forum channels, and channel log debugging.
---

# channel

`moluoxixi channel` is the local multi-agent collaboration runtime. Reach for it when agents need to talk through a durable event log, when a worker should be spawned as a peer process, when an in-flight worker needs interrupt / debugging, or when feedback should be recorded on a durable `--type forum` channel.
...
```bash
moluoxixi --version
moluoxixi channel --help
moluoxixi channel list --all
moluoxixi channel list --scope global --all
```

If the user names a channel or thread, inspect it before asking for background:

```bash
moluoxixi channel forum <board> --scope global
moluoxixi channel thread <board> <thread> --scope global
moluoxixi channel context list <board> --scope global --thread <thread>
```
...
- Use `--context-file` / `--context-raw` and `moluoxixi channel context add/delete/list`. `--linked-context-*` is deprecated terminology.
...
- The bundled Moluoxixi runtime owns reusable channel/thread state, event append, sequence allocation, context/title projection, reducers, task helpers, terminal rendering, prompts, worker lifecycle, and process exits.
...
- Long-term memory retrieval. Use durable forum channels for actionable issues, and `moluoxixi mem` (the `session-insight` skill) for session/history search.
