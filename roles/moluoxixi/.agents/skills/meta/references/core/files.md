...
**Purpose**: Track template file versions for `moluoxixi update`.

**Created by**: `moluoxixi init` or `moluoxixi update`

**Format**: JSON object mapping file paths to SHA-256 hashes.

```json
{
  ".moluoxixi/workflow.md": "028891d1fe839a266...",
...
**Used by**:
- `moluoxixi update` - Detect which files have been modified
...
**Used by**:
- `moluoxixi update` - Determine if update is needed
...
These files are managed by `moluoxixi update`:
