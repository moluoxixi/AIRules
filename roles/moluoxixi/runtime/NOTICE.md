...
`vendor/channel-mem.mjs` contains the Trellis v0.6.7 channel and memory implementations migrated from revision `e7c5ead4d0dfd717d11a40b6bc0c80d8af94c49a`. The corresponding TypeScript source is under `source/` and remains licensed under AGPL-3.0-only; the license and copyright texts are under `../skills/init-project/assets/trellis-v0.6.7/legal/`. `source/packages/cli/src/commands/channel/spawn.ts` is modified so detached supervisors re-enter the AIRules bundle. Migration and modification date: 2026-07-16.
