...
function update(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--dry-run' || arg === '--force') {
      ...
      process.stdout.write('Usage: trellis.mjs update [--platform <ids>] [--dry-run] [--force] [--python <command>]\n')
      return
    }
    else {
...
function workflow(args) {
  ...
  if (options.help) {
    process.stdout.write('Usage: trellis.mjs workflow [--list] [--template native|<local.md>] [--force] [--create-new]\n')
    return
  }
  if (options.list) {
    process.stdout.write('native\tAIRules-migrated Trellis workflow\nlocal-file\tPass a local Markdown path to --template\n')
    return
  }
  ...
  const nativeTemplate = path.join(projectRoot, PROJECT_ROOT_DIR, 'runtime', 'update', 'init-project', 'assets', 'trellis-v0.6.7', 'templates', 'trellis', 'workflow.md')
