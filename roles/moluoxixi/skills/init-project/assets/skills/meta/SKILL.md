...
- `.moluoxixi/workflow.md` is the local workflow source of truth; its initial content was selected from a workflow template (built-in `native`, `tdd`, `channel-driven-subagent-dispatch`, or a marketplace template) by the `init-project` skill and can be re-selected via `node "<skill-root>/scripts/moluoxixi.mjs" workflow --template <id>`. Missing `.moluoxixi/agents/<name>.md` files referenced by the active template trigger a non-blocking stderr warning pointing at `node "<skill-root>/scripts/moluoxixi.mjs" update`.
...
- `.moluoxixi/spec/` stores the user's project-specific coding conventions and design constraints. When `registry.spec` is set, files are refreshed by `node "<skill-root>/scripts/moluoxixi.mjs" update`; local edits surface as "modified by user" conflicts in `.moluoxixi/airules-init-manifest.json`.
