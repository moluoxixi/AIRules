...
Often the user does not need to change Trellis mechanics; they need local AI to understand their team's conventions. In that case, prefer `.moluoxixi/spec/` or a project-local skill instead of editing `meta`.
...
`meta` is a public skill for understanding Trellis architecture and local customization entry points. Put project-private content in:
...
This prevents future updates to Trellis's built-in `meta` from overwriting the team's own conventions.
