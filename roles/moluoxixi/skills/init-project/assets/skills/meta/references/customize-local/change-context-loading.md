...
2. `.moluoxixi/scripts/get_context.py`
3. `.moluoxixi/scripts/common/session_context.py`
4. `.moluoxixi/scripts/common/task_context.py`
5. `.moluoxixi/scripts/common/active_task.py`
...
| Source | Purpose |
| --- | --- |
| `.moluoxixi/workflow.md` | Workflow and next-action hints. |
| `.moluoxixi/tasks/<task>/prd.md` | Current task requirements. |
| `.moluoxixi/tasks/<task>/design.md` | Complex task technical design. |
| `.moluoxixi/tasks/<task>/implement.md` | Complex task execution plan. |
| `.moluoxixi/tasks/<task>/implement.jsonl` | Spec/research to read before implementation. |
| `.moluoxixi/tasks/<task>/check.jsonl` | Spec/research to read during checking. |
| `.moluoxixi/spec/` | Project specs. |
| `.moluoxixi/workspace/` | Session records. |
...
| Change hints on each user input | `[workflow-state:STATUS]` block in `.moluoxixi/workflow.md`. The `inject-workflow-state` hook is parser-only and reads the block verbatim. |
...
```jsonl
{"file": ".moluoxixi/spec/backend/index.md", "reason": "Backend conventions"}
{"file": ".moluoxixi/tasks/04-28-x/research/api.md", "reason": "API research"}
```
...
- `.moluoxixi/scripts/common/session_context.py`
...
python3 ./.moluoxixi/scripts/task.py list-context <task>
