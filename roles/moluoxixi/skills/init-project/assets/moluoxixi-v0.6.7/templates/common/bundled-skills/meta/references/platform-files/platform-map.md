...
| Pi Agent | `--pi` | `.pi/` | `.pi/skills/` | `.pi/agents/` | `.pi/extensions/moluoxixi/` (native `moluoxixi_subagent` tool) + `.pi/settings.json` |
...
Codex and Gemini CLI write the shared `.agents/skills/` layer. Some tools that support agentskills.io can also read this directory. If the user wants multiple compatible tools to share one skill, consider `.agents/skills/` first, but do not assume every platform reads it. ZCode keeps Moluoxixi-managed skills under `.zcode/skills/`.
