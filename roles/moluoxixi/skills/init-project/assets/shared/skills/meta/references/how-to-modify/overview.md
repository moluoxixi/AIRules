...
Common Moluoxixi customization scenarios and what files need to be modified.
...
|------|-----------------|----------|
| [Add slash command](#add-slash-command) | commands/, project-local | All |
| [Add agent](#add-agent) | agents/, hook, jsonl, project-local | CC |
| [Modify hook](#modify-hook) | hooks/, settings.json, project-local | CC |
| [Add spec category](#add-spec-category) | spec/, jsonl, project-local | All |
| [Change verify commands](#change-verify-commands) | worktree.yaml | CC |
| [Add workflow phase](#add-workflow-phase) | task.json, dispatch, project-local | CC |
| [Add post_create step](#add-post_create-step) | worktree.yaml | CC |
| [Modify session start](#modify-session-start) | session-start.py, project-local | CC |
| [Add core script](#add-core-script) | scripts/, project-local | All |
...
.moluoxixi/tasks/{template}/my-agent.jsonl            # Create: Context template
.project-local/SKILL.md                             # Update: Document the change
```
...
```
.moluoxixi/worktree.yaml                    # Modify: verify section
```
...
```
.moluoxixi/worktree.yaml                    # Modify: post_create section
```
...
```
.moluoxixi/scripts/my-script.py             # Create: Script
.moluoxixi/scripts/common/*.py              # Create/Modify: If shared utilities
.project-local/SKILL.md                   # Update: Document the change
```
...
```
.moluoxixi/scripts/task.py                  # Modify: init-context logic
.moluoxixi/tasks/{template}/*.jsonl         # Create: New JSONL templates
.project-local/SKILL.md                   # Update: Document the change
```
