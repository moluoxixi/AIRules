...
def resolve_task_dir(target_dir: str, repo_root: Path) -> Path | None:
    ...
    Returns:
        Resolved absolute path, or None when the target resolves outside the
        repository. Both sides are resolved so ``..`` and symlink escapes are
        checked against the real project root.
    """
    if not target_dir:
        return Path()
...
if __name__ == "__main__":
    ...
    print(f"is_safe_task_path('.moluoxixi/tasks/test'): {is_safe_task_path('.moluoxixi/tasks/test', repo)}")
