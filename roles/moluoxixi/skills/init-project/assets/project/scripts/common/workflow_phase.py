...
Extracts step-level content from .moluoxixi/workflow.md and optionally filters
...
def resolve_effective_platform(platform: str, config: dict) -> str:
    ...
    default or ``"codex-inline"`` when explicitly configured. ``sub-agent``
    remains a backwards-compatible alias for ``auto``.
    ...
    Native SubagentStart context injection supports the ``auto`` default.
    Invalid explicit values fall back to ``inline`` safely.
