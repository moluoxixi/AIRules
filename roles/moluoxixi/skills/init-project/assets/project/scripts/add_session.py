...
def is_git_worktree(repo_root: Path) -> bool:
    """Return True when repo_root is a linked worktree."""
    ...
    if rc_dir != 0 or rc_common != 0:
        return False
    return (repo_root / git_dir.strip()).resolve() != (repo_root / git_common_dir.strip()).resolve()


def warn_if_parallel_worktree(repo_root: Path) -> None:
    """Explain the one remaining journal conflict surface in linked worktrees."""
    ...
    print(
        colored(
            ...
            "journal-*.md files auto-merge via .gitattributes, but "
            ".moluoxixi/workspace/*/index.md conflicts across parallel worktrees "
            "are expected and safe to resolve by picking either side (task state "
            "lives in task.json, not index.md).",
            Colors.YELLOW,
        ),
        file=sys.stderr,
    )
...
def _render_bullet_section(header: str, items: list[str], bullet_prefix: str = "- ") -> str:
    """Render a Markdown section, omitting it when no values were supplied."""
    if not items:
        return ""
...
def _auto_commit_workspace(repo_root: Path) -> None:
    """Stage Moluoxixi-owned workspace + current-task paths and commit.
    ...
    via ``get_current_task``). We never `git add` the whole `.moluoxixi/` tree
    ...
    Honors ``session_auto_commit`` in ``.moluoxixi/config.yaml``: when set to
    ...
    # is ``.moluoxixi/tasks/<name>`` (or under archive/) — pass the bare name.
