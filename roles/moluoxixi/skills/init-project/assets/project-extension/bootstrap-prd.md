...
开发者刚刚在本项目中首次运行了 `moluoxixi init`。`.moluoxixi/` 已包含规范模板，
本任务用于把团队真实采用的开发约定整理到 `.moluoxixi/spec/`。后续
`moluoxixi-implement` 与 `moluoxixi-check` 子代理会自动读取这些规范；规范越贴近
现有代码，生成和检查结果越符合项目实际。
...
2. 使用 `moluoxixi-spec-bootstrap` 分析代码，并为每种模式寻找 2-3 个真实示例。
...
“Moluoxixi 已初始化完成。我会先读取仓库现有约定和真实代码，再和你确认需要补充
