...
function findProjectRoot(startDir: string): string | null {
   ...
   while (true) {
      if (existsSync(join(current, ".moluoxixi"))) return current;
...
function deriveContextKey(ctx?: { sessionManager?: { getSessionId?: () => string; getSessionFile?: () => string } }): string | null {
   ...
   const override = process.env.MOLUOXIXI_CONTEXT_ID?.trim();
...
function resolveActiveTaskStatus(
   ...
   const sessionsDir = join(projectRoot, ".moluoxixi", ".runtime", "sessions");
...
function buildSessionContext(projectRoot: string, contextKey: string | null): string {
   const script = join(projectRoot, ".moluoxixi", "scripts", "get_context.py");
   ...
   try {
      const result = spawnSync("python3", [script], {
         ...
         encoding: "utf-8",
         env: contextKey
            ? { ...process.env, MOLUOXIXI_CONTEXT_ID: contextKey }
            : process.env,
...
type AgentType = "moluoxixi-implement" | "moluoxixi-check" | "moluoxixi-research" | null;
...
function buildTaskContext(projectRoot: string, taskDir: string, agentType?: AgentType): string {
   ...
   if (agentType === "moluoxixi-implement") {
      jsonlNames = ["implement.jsonl"];
   } else if (agentType === "moluoxixi-check") {
      jsonlNames = ["check.jsonl"];
   } else if (agentType === "moluoxixi-research") {
...
const SESSION_OVERVIEW_TEXT =
   "Moluoxixi workflow system active. Use skills and agents as directed by the workflow state.";
...
class TurnContextCache {
   ...
   get(projectRoot: string, contextKey: string | null): { workflowMsg: string } {
      ...
      const workflowPath = join(projectRoot, ".moluoxixi", "workflow.md");
      let workflowMd = "";
...
const MOLUOXIXI_AGENTS = new Set(["moluoxixi-implement", "moluoxixi-check", "moluoxixi-research"]);

function detectAgentType(): AgentType {
   const blocked = process.env.PI_BLOCKED_AGENT;
   if (blocked && MOLUOXIXI_AGENTS.has(blocked)) {
...
export default function(pi: ExtensionAPI): void {
   ...
   pi.on("session_start", async (_event, ctx) => {
      ...
      if (isSubAgent) {
         ...
         if (sessionContext) {
            await pi.sendMessage({
               customType: "moluoxixi-session-context",
         ...
         ctx.ui.notify("Moluoxixi workflow system available", "info");
      }
   });
   ...
   pi.on("context", async (event, ctx) => {
      ...
      for (let i = messages.length - 1; i >= 0; i--) {
         if (messages[i].role === "custom" && messages[i].customType === "moluoxixi-workflow-state") {
            lastInjectionTs = Date.now();
            return;
         }
      }
