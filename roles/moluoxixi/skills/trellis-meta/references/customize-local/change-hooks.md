...
3. Read the hook script and identify inputs, outputs, and called `.moluoxixi/scripts/`.
4. Modify hook behavior.
5. If the hook depends on workflow content, synchronize `.moluoxixi/workflow.md`.
...
If the hook ultimately calls `.moluoxixi/scripts/get_context.py` or `session_context.py`, editing the local script is usually more robust than hard-coding content in the hook.
...
- Hooks should read project-local `.moluoxixi/`; they should not depend on Trellis upstream source paths.
