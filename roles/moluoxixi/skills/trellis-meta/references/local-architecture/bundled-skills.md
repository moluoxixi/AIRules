...
The `init-project` writer also places project-facing Trellis skills into each selected platform root. It uses the same local `trellis-channel`, `trellis-meta`, and `trellis-session-insight` trees that AIRules distributes at role level, so their runtime commands and references stay aligned.
