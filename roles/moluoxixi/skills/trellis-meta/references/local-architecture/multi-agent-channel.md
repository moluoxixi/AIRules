...
For command syntax, forum/thread patterns, worker handles, progress inspection, and the `--kind done` / `--kind turn_finished` dispatcher wait pattern, load the bundled `trellis-channel` skill (auto-installed under each platform's skills directory after the `init-project` skill / `node "<skill-root>/scripts/trellis.mjs" update`). This reference only covers the local file layout and customization knobs; it does not duplicate command syntax that may change between releases.
