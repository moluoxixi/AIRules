...
2. **Agent definitions** inside the project at `.trellis/agents/`: platform-agnostic role cards consumed by `node "<skill-root>/scripts/trellis.mjs" channel spawn --agent <name>`.
...
| Add a new role card | Drop `<name>.md` into `.trellis/agents/`. `node "<skill-root>/scripts/trellis.mjs" channel spawn --agent <name>` will pick it up. |
...
- **Workflow layer**: workflows that use channel dispatch (such as `channel-driven-subagent-dispatch`) instruct the main agent to call `node "<skill-root>/scripts/trellis.mjs" channel spawn --agent check` or `--agent implement` instead of a platform sub-agent. If `.trellis/agents/check.md` or `implement.md` is missing, `node "<skill-root>/scripts/trellis.mjs" workflow --template <id>` prints a non-blocking warning at install time. Restore them with `node "<skill-root>/scripts/trellis.mjs" update` if they are deleted by accident.
