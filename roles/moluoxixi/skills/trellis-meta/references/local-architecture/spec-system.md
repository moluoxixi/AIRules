...
`.moluoxixi/spec/` is the user's project-specific engineering spec library. Trellis is not about making AI memorize conventions; it injects relevant specs or requires the AI to read them at the right time.
...
| Change when specs should be updated | Phase 3.3 in `.moluoxixi/workflow.md` and the `trellis-update-spec` skill. |

## Boundaries

`.moluoxixi/spec/` is the user's project specification, not a permanent copy of Trellis built-in templates. The AI should encourage the user to update it according to the actual project code instead of treating Trellis default templates as immutable documents.
