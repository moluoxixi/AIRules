...
the `init-project` skill writes the Trellis runtime into the user project. Later, `node "<skill-root>/scripts/trellis.mjs" update` tries to update Trellis-managed template files, but it uses `.trellis/airules-init-manifest.json` to determine which files have already been modified by the user.
...
| `.trellis/runtime/` | No | AIRules-owned channel, memory, workflow, and updater runtime plus corresponding source. |
...
| `.trellis/airules-init-manifest.json` | No | Template hash record. Do not hand-write business rules here. |
...
`.trellis/airules-init-manifest.json` records the content hash from the last time Trellis wrote a template file. `node "<skill-root>/scripts/trellis.mjs" update` uses it to distinguish three cases:
...
- `.trellis/runtime/**` executable and updater assets
- Concrete state files under `.trellis/.runtime/**`
- Hash contents inside `.trellis/airules-init-manifest.json`
