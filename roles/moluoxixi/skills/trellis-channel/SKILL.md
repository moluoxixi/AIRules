...
```bash
node "<skill-root>/scripts/trellis.mjs" --version
node "<skill-root>/scripts/trellis.mjs" channel --help
node "<skill-root>/scripts/trellis.mjs" channel list --all
node "<skill-root>/scripts/trellis.mjs" channel list --scope global --all
```
...
node "<skill-root>/scripts/trellis.mjs" channel thread <board> <thread> --scope global
...
- Use `--context-file` / `--context-raw` and `node "<skill-root>/scripts/trellis.mjs" channel context add/delete/list`. `--linked-context-*` is deprecated terminology.
...
- Long-term memory retrieval. Use durable forum channels for actionable issues, and `node "<skill-root>/scripts/trellis.mjs" mem` (the `trellis-session-insight` skill) for session/history search.
