...
```bash
node "<skill-root>/scripts/trellis.mjs" channel create design-feedback \
  --type forum \
  --scope global \
...
```bash
node "<skill-root>/scripts/trellis.mjs" channel post design-feedback opened \
  --scope global \
  --as main \
...
node "<skill-root>/scripts/trellis.mjs" channel post design-feedback comment \
  --scope global \
  --as reviewer \
  --thread login-empty-state \
  --text-file /tmp/review.md

node "<skill-root>/scripts/trellis.mjs" channel post design-feedback status \
  --scope global \
  --as main \
  --thread login-empty-state \
  --status closed

node "<skill-root>/scripts/trellis.mjs" channel post design-feedback summary \
  --scope global \
  --as main \
  --thread login-empty-state \
...
```bash
node "<skill-root>/scripts/trellis.mjs" channel messages design-feedback --scope global
node "<skill-root>/scripts/trellis.mjs" channel forum design-feedback --scope global --status open
node "<skill-root>/scripts/trellis.mjs" channel thread design-feedback login-empty-state --scope global
node "<skill-root>/scripts/trellis.mjs" channel messages design-feedback --scope global --raw --thread login-empty-state
```
...
```bash
node "<skill-root>/scripts/trellis.mjs" channel context list design-feedback --scope global
node "<skill-root>/scripts/trellis.mjs" channel context list design-feedback --scope global --thread login-empty-state --raw
```
...
```bash
node "<skill-root>/scripts/trellis.mjs" channel context delete design-feedback \
  --scope global \
  --thread login-empty-state \
  --raw "stale note"
```
...
```bash
node "<skill-root>/scripts/trellis.mjs" channel title set design-feedback \
  --scope global \
  --title "Design feedback board"

node "<skill-root>/scripts/trellis.mjs" channel title clear design-feedback --scope global
```
...
```bash
node "<skill-root>/scripts/trellis.mjs" channel thread rename design-feedback old-key new-key \
  --scope global \
  --as main
```
...
```bash
node "<skill-root>/scripts/trellis.mjs" channel create release-notes \
  --type forum \
  --scope global \
...
node "<skill-root>/scripts/trellis.mjs" channel post release-notes opened \
  --scope global \
  --as main \
