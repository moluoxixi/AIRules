...
`node "<skill-root>/scripts/trellis.mjs" channel messages <channel>` renders a compact, human-readable view:
...
node "<skill-root>/scripts/trellis.mjs" channel messages <channel> --kind done --last 10
node "<skill-root>/scripts/trellis.mjs" channel messages <channel> --kind error --last 10

# Raw (diagnostic view) — one JSON per line
node "<skill-root>/scripts/trellis.mjs" channel messages <channel> --raw --kind progress --last 20
node "<skill-root>/scripts/trellis.mjs" channel messages <channel> --raw --last 50
```
...
```bash
node "<skill-root>/scripts/trellis.mjs" channel messages <channel> --raw --kind progress --last 80 \
...
Symptom: `node "<skill-root>/scripts/trellis.mjs" channel list` shows the worker as running, but no new
...
2. **Confirm the supervisor and worker PIDs are alive.**
   ...
   `node "<skill-root>/scripts/trellis.mjs" channel kill <name> --as <worker> --force`.
...
```bash
node "<skill-root>/scripts/trellis.mjs" channel wait T --as main --from check --kind done --timeout 15m
node "<skill-root>/scripts/trellis.mjs" channel wait T --as main --from check,check-cx --kind done --all --timeout 15m
node "<skill-root>/scripts/trellis.mjs" channel wait T --as worker --tag interrupt --timeout 1h
node "<skill-root>/scripts/trellis.mjs" channel wait T --as main --thread release-note --action status --timeout 10m
```
...
node "<skill-root>/scripts/trellis.mjs" channel forum list <channel>

# Inspect one thread end-to-end
node "<skill-root>/scripts/trellis.mjs" channel thread show <channel> <thread>

# Replay messages for a thread (supports --raw, --kind, --last)
node "<skill-root>/scripts/trellis.mjs" channel messages <channel> --thread <thread> --raw --last 100

# What a specific worker still has pending
node "<skill-root>/scripts/trellis.mjs" channel context <channel> --as <worker>
```
...
| Symptom | Cause | Fix |
|---|---|---|
| `trellis: command not found` | CLI not installed globally | `node "<skill-root>/scripts/trellis.mjs" --version` |
...
| ghost worker in list | supervisor died without cleanup | `node "<skill-root>/scripts/trellis.mjs" channel kill <name> --as <worker> --force` |
