...
Run `python3 ./.moluoxixi/scripts/task.py current --source` → active task path. If no active task is set, ask the user where to write output; do NOT guess.
...
- Code files (`src/`, `lib/`, …)
- Spec files (`.moluoxixi/spec/`) — main agent should use `update-spec` skill instead
- `.moluoxixi/scripts/`, `.moluoxixi/workflow.md`, platform config (`.claude/`, `.cursor/`, etc.)
...
- `.moluoxixi/spec/xxx.md` — <description>
