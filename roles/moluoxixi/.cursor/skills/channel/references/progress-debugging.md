...
`moluoxixi channel messages <channel>` renders a compact, human-readable view:
...
moluoxixi channel messages <channel> --kind done --last 10
moluoxixi channel messages <channel> --kind error --last 10

# Raw (diagnostic view) — one JSON per line
moluoxixi channel messages <channel> --raw --kind progress --last 20
moluoxixi channel messages <channel> --raw --last 50
```
...
```bash
moluoxixi channel messages <channel> --raw --kind progress --last 80 \
...
Symptom: `moluoxixi channel list` shows the worker as running, but no new
...
1. **Locate the channel files.** Use `list --all --all-projects` if you are
   ...
   ```bash
   moluoxixi channel list --all --all-projects
...
2. **Confirm the supervisor and worker PIDs are alive.**
   ...
   `moluoxixi channel kill <name> --as <worker> --force`.
...
```bash
moluoxixi channel wait T --as main --from check --kind done --timeout 15m
moluoxixi channel wait T --as main --from check,check-cx --kind done --all --timeout 15m
moluoxixi channel wait T --as worker --tag interrupt --timeout 1h
moluoxixi channel wait T --as main --thread release-note --action status --timeout 10m
```
...
moluoxixi channel forum list <channel>

# Inspect one thread end-to-end
moluoxixi channel thread show <channel> <thread>

# Replay messages for a thread (supports --raw, --kind, --last)
moluoxixi channel messages <channel> --thread <thread> --raw --last 100

# What a specific worker still has pending
moluoxixi channel context <channel> --as <worker>
```
...
| Symptom | Cause | Fix |
|---|---|---|
| `moluoxixi: command not found` | A global CLI was invoked instead of the project runtime | run `node .moluoxixi/runtime/moluoxixi.mjs <command>` from the project root |
...
| ghost worker in list | supervisor died without cleanup | `moluoxixi channel kill <name> --as <worker> --force` |
