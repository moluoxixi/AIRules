...
For command syntax, forum/thread patterns, worker handles, progress inspection, and the `--kind done` / `--kind turn_finished` dispatcher wait pattern, load the bundled `channel` skill (auto-installed under each platform's skills directory after `moluoxixi init` / `moluoxixi update`). This reference only covers the local file layout and customization knobs; it does not duplicate command syntax that may change between releases.
