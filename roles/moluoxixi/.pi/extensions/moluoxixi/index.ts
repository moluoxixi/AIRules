...
// ── Constants ─────────────────────────────────────────────────────────
const MOLUOXIXI_AGENT_JSONL: Record<string, string> = {
...
interface ProgressDetails {
  kind: "moluoxixi-subagent-progress";
  agent: string;
  mode: "single" | "parallel" | "chain";
  startedAt: number;
  updatedAt: number;
...
function cmdHasMoluoxixiCtx(cmd: string) {
  const t = cmd.trimStart();
  return (
    /^export\s+MOLUOXIXI_CONTEXT_ID=/.test(t) ||
    /^MOLUOXIXI_CONTEXT_ID=/.test(t) ||
    /^env\s+.*MOLUOXIXI_CONTEXT_ID=/.test(t)
  );
}
...
function resolvePiCli(): { command: string; args: string[] } {
  const envCli = str(process.env.MOLUOXIXI_PI_CLI_JS);
  if (envCli) {
    const p = resolve(envCli);
    if (!exists(p)) throw new Error(`MOLUOXIXI_PI_CLI_JS missing: ${p}`);
...
function findRoot(start: string): string {
  let c = resolve(start);
  while (true) {
    if (existsSync(join(c, ".moluoxixi")) || existsSync(join(c, ".pi"))) return c;
    const p = dirname(c);
...
function contextKey(input?: unknown, ctx?: PiExtensionContext): string | null {
  const ov = str(process.env.MOLUOXIXI_CONTEXT_ID);
...
function readTaskDir(root: string, key: string | null): string | null {
  ...
  try {
    const ctx = JSON.parse(
      readText(join(root, ".moluoxixi", ".runtime", "sessions", `${key}.json`)),
    ) as JsonObject;
    let ref = str(ctx.current_task);
    if (!ref) return null;
    ref = ref;
    ref = ref.replace(/\\/g, "/").replace(/^\.\//, "");
    if (ref.startsWith("tasks/")) ref = `.moluoxixi/${ref}`;
    return ref.startsWith(".moluoxixi/")
      ? join(root, ref)
      : isAbsolute(ref)
        ? ref
        : join(root, ".moluoxixi", "tasks", ref);
  } catch {
    return null;
  }
}
...
function adoptKey(root: string, key: string): string {
  ...
  try {
    const dir = join(root, ".moluoxixi", ".runtime", "sessions");
...
function workflowBreadcrumb(root: string, key: string | null): string {
  const wf = readText(join(root, ".moluoxixi", "workflow.md"));
  if (!wf) return "";
...
function sessionOverview(root: string, key: string | null): string {
  const script = join(root, ".moluoxixi", "scripts", "get_context.py");
  if (!exists(script)) return "";
  try {
    const py = process.platform === "win32" ? "python" : "python3";
    const result = spawnSync(py, [script], {
      cwd: root,
      env: key ? { ...process.env, MOLUOXIXI_CONTEXT_ID: key } : process.env,
      encoding: "utf-8",
...
function normalizeAgent(agent: string | undefined): string {
  const name = agent ?? "moluoxixi-implement";
  return name.startsWith("moluoxixi-") ? name : `moluoxixi-${name}`;
}

function isMoluoxixiAgent(root: string, agent: string): boolean {
...
function runPi(
  ...
  return new Promise((resolve) => {
    ...
    const childEnv = {
      ...process.env,
      MOLUOXIXI_SUBAGENT_CHILD: "1",
      ...(key ? { MOLUOXIXI_CONTEXT_ID: key } : {}),
    };
...
// ── Extension ──────────────────────────────────────────────────────────
export default function moluoxixiExtension(pi: {
  ...
  if (process.env.MOLUOXIXI_SUBAGENT_CHILD === "1") return;
  ...
  // Tool registration
  pi.registerTool?.({
    name: "moluoxixi_subagent",
    label: "Moluoxixi Subagent",
    description: "Run a Moluoxixi project sub-agent with active task context.",
    promptSnippet:
      'Sub-agent dispatch protocol (Moluoxixi): your dispatch prompt MUST start with one line "Active task: <task path from `task.py current`>" before any other instructions.',
    promptGuidelines: [
    ...
    parameters: {
      ...
      properties: {
        agent: {
          type: "string",
          description:
            "Agent name, such as moluoxixi-implement or moluoxixi-check.",
        },
        prompt: {
          type: "string",
    ...
    execute: async (
      ...
      activeSubagentToolCallId = id;
      const agentName = normalizeAgent(input.agent);
      if (!isMoluoxixiAgent(root, agentName)) {
        return {
          content: [
            {
              type: "text",
              text:
                "`moluoxixi_subagent` is only for Moluoxixi workflow agents with a definition file in .pi/agents/.\n\n" +
                ...
                `- Create .pi/agents/${agentName}.md for your custom Moluoxixi agent\n` +
                "- Install a community subagent package: pi install -l npm:@tintinweb/pi-subagents",
            },
          ],
          details: { agent: agentName, error: "not a moluoxixi workflow agent" },
        };
      }
      const mode = input.mode ?? "single";
    ...
    renderResult: (
      ...
      const isProgress =
        ...
        result.details.kind === "moluoxixi-subagent-progress";
  ...
  pi.on?.("session_start", (event, ctx) => {
    getKey(event, ctx);
    ctx?.ui?.notify?.(
      "Moluoxixi project context is available. Use /moluoxixi-continue to resume the current task.",
      "info",
    );
  });
  ...
  pi.on?.("tool_call", (event, ctx) => {
    ...
    if (
      ...
      !cmdHasMoluoxixiCtx(ev.input.command)
    )
      ev.input.command = `export MOLUOXIXI_CONTEXT_ID=${shellQuote(k)}; ${ev.input.command}`;
  });
  ...
  pi.on?.("tool_result", (event) => {
    ...
    if (
      ev.toolName === "moluoxixi_subagent" &&
      isObj(ev.details) &&
      ev.details.kind === "moluoxixi-subagent-progress" &&
  ...
  pi.on?.("before_agent_start", (event, ctx) => {
    ...
    const ctxText = buildContext(root, "moluoxixi-implement", k);
