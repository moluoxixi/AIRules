 ...
 * task.json shape.
 */
export interface MoluoxixiTaskRecord {
  id: string;
  name: string;
  title: string;
  description: string;
...
export const TASK_RECORD_FIELD_ORDER = [
  ...
  "notes",
  "meta",
] as const satisfies readonly (keyof MoluoxixiTaskRecord)[];
...
/**
 * Lightweight runtime schema for {@link MoluoxixiTaskRecord}. Zero-dep on
...
export const taskRecordSchema = {
  parse(input: unknown): MoluoxixiTaskRecord {
    return parseTaskRecord(input);
  },
  safeParse(
    input: unknown,
  ):
    | { success: true; data: MoluoxixiTaskRecord }
...
function parseTaskRecord(input: unknown): MoluoxixiTaskRecord {
...
/**
 * Produce a fully-populated canonical-shape {@link MoluoxixiTaskRecord}.
 *
...
export function emptyTaskRecord(
  overrides: Partial<MoluoxixiTaskRecord> = {},
): MoluoxixiTaskRecord {
  const today = new Date().toISOString().split("T")[0] ?? "";
  const base: MoluoxixiTaskRecord = {
    id: "",
    name: "",
    title: "",
    description: "",
