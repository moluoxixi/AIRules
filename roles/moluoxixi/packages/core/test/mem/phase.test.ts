...
const { fakeHome } = vi.hoisted(() => {
  ...
  const fakeHome = f.mkdtempSync(p.join(o.tmpdir(), "moluoxixi-mem-phase-"));
  return { fakeHome };
});
...
describe("parseTaskPyCommandsAll (dogfood-driven edge cases)", () => {
  it("strips $(...) closing paren from --slug value", () => {
    const all = parseTaskPyCommandsAll(
      'TASK_DIR=$(python3 ./.moluoxixi/scripts/task.py create "fix: moluoxixi mem --since drops cross-day sessions" --slug mem-since-cross-day-filter)',
    );
