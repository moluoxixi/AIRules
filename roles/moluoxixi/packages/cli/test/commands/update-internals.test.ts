...
describe("cleanupEmptyDirs", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "moluoxixi-cleanup-"));
  });
...
describe("renameTracesToJournal", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "moluoxixi-traces-"));
...
describe("classifyMigrations rename-dir ownership gate", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "moluoxixi-renamedir-"));
    // A user-owned .windsurf/workflows that Moluoxixi never created.
  ...
  it("auto-migrates when Moluoxixi owns the source dir (manifest has entries)", () => {
