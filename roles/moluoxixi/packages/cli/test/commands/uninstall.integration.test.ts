...
describe("uninstall() integration", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "moluoxixi-uninstall-int-"));
  ...
  it("#1 friendly exit when .moluoxixi/ is missing", async () => {
  ...
  it("#2 errors when manifest is missing but .moluoxixi/ exists", async () => {
  ...
  it("#3 init → uninstall → project is clean", async () => {
    ...
    // package.json) may legitimately remain when the moluoxixi template
    // shipped non-moluoxixi fields too (e.g. .claude/settings.json's `env`
  ...
  it("#6 user-modified moluoxixi file is still deleted (manifest defines scope)", async () => {
  ...
  it("#8a empty managed sub-dirs and root dir are pruned (kilo: no structured config)", async () => {
    ...
    const kiloEntry = Object.keys(hashesBefore).find(
      (p) => !p.startsWith(".moluoxixi/") && p !== "AGENTS.md",
    );
  ...
  it("#8b platform root dir survives only when scrubbing leaves residual structured content", async () => {
    ...
    // After moluoxixi hooks are stripped, `{ version: 1 }` remains — not fully
  ...
  it("#8 .claude/settings.json with extra user fields keeps user fields, strips moluoxixi hooks", async () => {
    await init({ yes: true, claude: true, force: true });

    // Simulate a user editing settings.json to add custom fields and a custom
    // hook entry alongside the moluoxixi ones.
    ...
    // .moluoxixi/ is gone, but settings.json should remain (had user fields).
    if (fs.existsSync(settingsPath)) {
      ...
      // User hook (if it was inserted) should still be present, moluoxixi ones gone.
