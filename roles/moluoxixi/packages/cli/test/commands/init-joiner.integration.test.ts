 ...
 *   no .moluoxixi/                        → creator bootstrap task
 *   .moluoxixi/ exists, .developer missing → joiner onboarding task
...
describe("init() joiner onboarding", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "moluoxixi-joiner-int-"));
  ...
  /**
   * Helper: simulate a fresh clone of an existing Moluoxixi project — `.moluoxixi/`
  ...
  /** Helper: simulate same-dev re-init — both `.moluoxixi/` and `.developer` exist */
  ...
  it("#2 existing .moluoxixi/ + no .developer → joiner onboarding task created", async () => {
    simulateExistingCheckout();
    ...
    expect(prd).toContain(".moluoxixi/spec/");
    ...
    expect(prd).toContain(
      `${expectedPythonCmd} ./.moluoxixi/scripts/task.py list --assignee bob`,
    );
    expect(prd).toContain(`${expectedPythonCmd} ./.moluoxixi/scripts/task.py finish`);
    expect(prd).toContain(
      `${expectedPythonCmd} ./.moluoxixi/scripts/task.py archive 00-join-bob`,
    );
  ...
  it("#2b issue #204: existing .moluoxixi/ but tasks/ empty → bootstrap fallback (--yes alone, no --force)", async () => {
    // Mirrors the exact reproduction in issue #204: first run aborted partway
    // after writing the .moluoxixi/ skeleton but before creating bootstrap;
  ...
  it("#3 existing .moluoxixi/ + .developer → no task created", async () => {
  ...
  it("#6 joiner creation failure surfaces as warning, init does not crash", async () => {
    ...
    // thwarted by making .moluoxixi/tasks read-only right before dispatch,
  ...
  // Tests #7/#8 cover the handleReinit path — the default flow when .moluoxixi/
  ...
  it("#7 handleReinit path: existing .moluoxixi/ + no .developer → joiner task created", async () => {
    simulateExistingCheckout();
  ...
  it("#8 handleReinit path: existing .moluoxixi/ + .developer → no task created", async () => {
