...
describe("requireSupportedPython", () => {
  ...
  it("throws when the detected version is below the supported floor", () => {
    ...
    expect(() => requireSupportedPython("python3")).toThrow(
      'Python 3.8.18 detected via "python3", but Moluoxixi init requires Python ≥ 3.9.',
    );
  });
  ...
  it("throws when the command is missing", () => {
    ...
    expect(() => requireSupportedPython("python")).toThrow(
      'Python command "python" not found. Moluoxixi init requires Python ≥ 3.9.',
    );
  });
  ...
  it("skips the probe entirely when MOLUOXIXI_SKIP_PYTHON_CHECK=1", () => {
    const prev = process.env.MOLUOXIXI_SKIP_PYTHON_CHECK;
    process.env.MOLUOXIXI_SKIP_PYTHON_CHECK = "1";
    try {
      // execSync should not be called at all
      const result = requireSupportedPython("python3");
      expect(result).toBe("version check skipped (MOLUOXIXI_SKIP_PYTHON_CHECK=1)");
      expect(execSync).not.toHaveBeenCalled();
    } finally {
      if (prev === undefined) {
        delete process.env.MOLUOXIXI_SKIP_PYTHON_CHECK;
      } else {
        process.env.MOLUOXIXI_SKIP_PYTHON_CHECK = prev;
      }
    }
  });
});
...
describe("resolveSupportedPython", () => {
  beforeEach(() => {
    vi.mocked(execSync).mockReset();
    resetResolvedPythonCommand();
    delete process.env.MOLUOXIXI_PYTHON_CMD;
    delete process.env.MOLUOXIXI_SKIP_PYTHON_CHECK;
  });
  ...
  it("honors MOLUOXIXI_PYTHON_CMD as an explicit override (no probe)", () => {
    process.env.MOLUOXIXI_PYTHON_CMD = "py -3.12";
  ...
  it("honors MOLUOXIXI_SKIP_PYTHON_CHECK=1 as last-resort escape hatch", () => {
    process.env.MOLUOXIXI_SKIP_PYTHON_CHECK = "1";
