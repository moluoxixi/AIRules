...
describe("init + uninstall: manifest accuracy + homedir guard", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "moluoxixi-overdelete-"));
  ...
  // ----- R1.6: Moluoxixi-written AGENTS.md with user content outside the block -----
  ...
  it("#R1.7 uninstall still deletes a tracked AGENTS.md that is only the Moluoxixi block", async () => {
