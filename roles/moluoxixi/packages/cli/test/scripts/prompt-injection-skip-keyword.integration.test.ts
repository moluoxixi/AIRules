...
function setupRepo(tmp: string): void {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "No active task. Classify the turn before creating a Moluoxixi task.",
      "[/workflow-state:no_task]",
      "",
      "## Phase 1: Plan",
      "",
...
describeFn("no-moluoxixi skip keyword (issue #427)", () => {
  ...
  beforeEach(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), "moluoxixi-skip-keyword-"));
    setupRepo(tmp);
  });
