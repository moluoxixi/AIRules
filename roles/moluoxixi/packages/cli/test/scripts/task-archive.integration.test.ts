 ...
 * `src/templates/moluoxixi/scripts/common/task_store.py`; this test stamps
...
function runArchive(repo: string, taskName: string): void {
  const r = spawnSync(
    "python3",
    [".moluoxixi/scripts/task.py", "archive", taskName],
...
describe.skipIf(!hasPython())(
  ...
  () => {
    ...
    beforeEach(() => {
      tmp = fs.mkdtempSync(path.join(os.tmpdir(), "moluoxixi-archive-test-"));
      setupRepo(tmp);
    });
    ...
    it(
      ...
      () => {
        ...
        expect(
          deletes.every((p) => p.startsWith(".moluoxixi/tasks/big/")),
        ).toBe(true);
      },
    ...
    it("refuses to archive a mistyped name that resolves to a real source dir", () => {
      ...
      // dir into .moluoxixi/tasks/archive/.
      const r = spawnSync(
        "python3",
        [".moluoxixi/scripts/task.py", "archive", "src"],
      ...
      expect(
        fs.existsSync(
          path.join(tmp, ".moluoxixi", "tasks", "archive"),
        ),
      ).toBe(false);
    });
    ...
    it("fails when archive auto-commit cannot record tracked source deletes", () => {
      ...
      const r = spawnSync(
        "python3",
        [".moluoxixi/scripts/task.py", "archive", "tracked"],
        { cwd: tmp, encoding: "utf-8" },
      );
      ...
      const status = git(tmp, "status", "--porcelain");
      expect(status).toContain(".moluoxixi/tasks/tracked/");
      expect(status).toContain(".moluoxixi/tasks/archive/");
    });
  },
);
