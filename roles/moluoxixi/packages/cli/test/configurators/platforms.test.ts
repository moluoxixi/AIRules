...
const BUNDLED_SKILL_NAME = "moluoxixi-meta";
...
// 0.5.5 shipped `.agents/skills/moluoxixi-start/SKILL.md` from `configureCodex`
// with no matching `collectTemplates` entry, leaving upgraders without the
// file after `moluoxixi update` (see manifests/0.5.7.json).
// =============================================================================
 ...
 * `moluoxixi init --with-statusline`. Keeping it out of `collectTemplates` is
 ...
 * `.moluoxixi/tasks/08-06-converge-platform-templates/research/configure-vs-collect-inventory.md`):
 ...
 * is frozen after their first `moluoxixi update` and is left behind by
 * `moluoxixi uninstall`.
 */
...
const CONFIGURE_ONLY_EMPTY_DIRS: Partial<Record<(typeof PLATFORM_IDS)[number], string[]>> =
  {
    // Moluoxixi ships no Codex-specific skills (they all land in
...
// =============================================================================
// getConfiguredPlatforms — detects Moluoxixi-owned platform files
// =============================================================================
...
describe("getConfiguredPlatforms", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "moluoxixi-platforms-"));
    setWriteMode("force");
  });
  ...
  it("does not treat native platform directories as Moluoxixi installations", () => {
  ...
  it("detects Moluoxixi-namespaced legacy Windsurf workflows as devin", () => {
    ...
    fs.writeFileSync(path.join(workflowsDir, "moluoxixi-continue.md"), "# Moluoxixi");
    const result = getConfiguredPlatforms(tmpDir);
    expect(result.has("devin")).toBe(true);
  });

  it("detects every platform from the files Moluoxixi tracked for it", async () => {
    for (const id of PLATFORM_IDS) {
      ...
      fs.mkdirSync(path.join(platformRoot, ".moluoxixi"), { recursive: true });
...
describe("configurePlatform", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "moluoxixi-configure-"));
  ...
  it("configurePlatform writes collected templates byte-for-byte for every platform", async () => {
    for (const id of PLATFORM_IDS) {
      const platformDir = fs.mkdtempSync(
        path.join(os.tmpdir(), `moluoxixi-parity-${id}-`),
      );
      try {
  ...
  it("configurePlatform writes no file collectTemplates does not describe, for every platform", async () => {
    ...
    // mode that shipped in 0.5.5 (codex moluoxixi-start).
    for (const id of PLATFORM_IDS) {
      const platformDir = fs.mkdtempSync(
        path.join(os.tmpdir(), `moluoxixi-reverse-${id}-`),
      );
      try {
        await configurePlatform(id, platformDir);
        const templates = collectPlatformTemplates(id);
  ...
  it("configurePlatform and collectTemplates agree under Windows python rendering", async () => {
    ...
    try {
      for (const id of PLATFORM_IDS) {
        const platformDir = fs.mkdtempSync(
          path.join(os.tmpdir(), `moluoxixi-win-${id}-`),
        );
        try {
          await configurePlatform(id, platformDir);
          const templates = collectPlatformTemplates(id);
  ...
  it("configurePlatform('codex') writes shared skill templates from common source", async () => {
    ...
    // `moluoxixi-start` is included via `resolveAllAsSkillsNeutral` directly —
    // it's the user-invocable fallback referenced by the <moluoxixi-bootstrap>
    ...
    expect(
      fs.existsSync(path.join(skillsRoot, "moluoxixi-start", "SKILL.md")),
    ).toBe(true);
  });
  ...
  it("configurePlatform('codex') writes custom agents and config", async () => {
    ...
    for (const agent of expectedAgents) {
      ...
      expect(written).toContain("<!-- moluoxixi-hook-injected -->");
      expect(written).toContain("Active task: <path>");
      expect(written).not.toContain("Required: Load Moluoxixi Context First");
    }
  ...
  it("configurePlatform('kiro') writes main agent, IDE hook, and shared hooks", async () => {
    ...
    // Main `moluoxixi` agent wires per-turn + session-start hooks; PYTHON_CMD
    // resolved.
    const moluoxixiPath = path.join(tmpDir, ".kiro", "agents", "moluoxixi.json");
    expect(fs.existsSync(moluoxixiPath)).toBe(true);
    const moluoxixiRaw = fs.readFileSync(moluoxixiPath, "utf-8");
    expect(moluoxixiRaw).not.toContain("{{PYTHON_CMD}}");
    const moluoxixi = JSON.parse(moluoxixiRaw) as {
      resources?: string[];
      hooks?: Record<string, { command: string }[]>;
    };
    expect(moluoxixi.hooks?.userPromptSubmit?.[0].command).toBe(
      `${expectedPythonCmd} .kiro/hooks/inject-workflow-state.py`,
    );
    expect(moluoxixi.hooks?.agentSpawn?.[0].command).toBe(
    ...
    const ideHookPath = path.join(hooksDir, "moluoxixi-workflow-state.kiro.hook");
  ...
  it("configurePlatform('gemini') writes TOML commands + SKILL.md skills", async () => {
    ...
    // Commands as TOML
    const commandsDir = path.join(tmpDir, ".gemini", "commands", "moluoxixi");
    ...
    for (const dir of skillDirs) {
      expect(
        dir.name.startsWith("moluoxixi-") || dir.name === "spec-review",
      ).toBe(true);
  ...
  it("configurePlatform('qoder') writes commands + skills with the correct split", async () => {
    ...
    expect(actualCommandFiles).toEqual(
      expectedCommands.map((c) => `moluoxixi-${c.name}.md`).sort(),
    );
    for (const cmd of expectedCommands) {
      const name = `moluoxixi-${cmd.name}`;
    ...
    expect(actualSkillDirs).not.toContain("moluoxixi-finish-work");
    expect(actualSkillDirs).not.toContain("moluoxixi-continue");
    expect(actualSkillDirs).not.toContain("moluoxixi-start");
  });
  ...
  it("configurePlatform('grok') writes flat commands and .grok agents", async () => {
    ...
    expect(
      fs.existsSync(path.join(tmpDir, ".grok", "commands", "moluoxixi-start.md")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".grok", "commands", "moluoxixi-continue.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".grok", "commands", "moluoxixi", "start.md"),
      ),
    ).toBe(false);
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".grok", "skills", "moluoxixi-check", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".grok", "agents", "moluoxixi-implement.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(tmpDir, ".grok", "agents", "moluoxixi-check.md")),
    ).toBe(true);
    const researchAgentPath = path.join(
      tmpDir,
      ".grok",
      "agents",
      "moluoxixi-research.md",
    );
    ...
    expect(templates?.has(".grok/commands/moluoxixi-start.md")).toBe(true);
    expect(templates?.has(".grok/commands/moluoxixi/start.md")).toBe(false);
    expect(
      [...(templates?.keys() ?? [])].some((key) =>
        key.startsWith(".agents/skills/"),
      ),
    ).toBe(false);
    expect(templates?.has(".grok/agents/moluoxixi-implement.md")).toBe(true);
    expect(templates?.has(".grok/agents/moluoxixi-research.md")).toBe(true);
  });
  ...
  it("configurePlatform('snow') writes class-1 inject hooks, skills, commands, and agents", async () => {
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".snow", "skills", "moluoxixi-check", "SKILL.md"),
      ),
    ).toBe(true);
    // hasHooks=true → moluoxixi-start is filtered out (session inject replaces it)
    expect(
      fs.existsSync(
        path.join(tmpDir, ".snow", "skills", "moluoxixi-start", "SKILL.md"),
      ),
    ).toBe(false);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".snow", "commands", "moluoxixi-start.json"),
      ),
    ).toBe(false);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".snow", "commands", "moluoxixi-continue.json"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".snow", "commands", "moluoxixi-finish-work.json"),
      ),
    ).toBe(true);
    ...
    expect(continueCmd.command).toContain(".moluoxixi");

    expect(
      fs.existsSync(
        path.join(tmpDir, ".snow", "agents", "moluoxixi-implement.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(tmpDir, ".snow", "agents", "moluoxixi-check.md")),
    ).toBe(true);
    const researchAgentPath = path.join(
      tmpDir,
      ".snow",
    ...
    ).not.toContain("Load Moluoxixi Context First");
    expect(
      fs.readFileSync(
        path.join(tmpDir, ".snow", "agents", "moluoxixi-implement.md"),
        "utf-8",
    ...
    // only Moluoxixi-owned files recorded in the manifest count as configured.
    const emptyDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "moluoxixi-snow-det-"),
    );
    try {
    ...
    expect(templates?.has(".snow/commands/moluoxixi-start.json")).toBe(false);
    expect(templates?.has(".snow/skills/moluoxixi-start/SKILL.md")).toBe(false);
    expect(templates?.has(".snow/commands/moluoxixi-continue.json")).toBe(true);
    expect(templates?.has(".snow/agents/moluoxixi-implement.md")).toBe(true);
    expect(templates?.has(".snow/agents/moluoxixi-research.md")).toBe(true);
    expect(templates?.has(".snow/sub-agents.moluoxixi.json")).toBe(false);
    ...
    expect(templates?.has(".snow/hooks/write-moluoxixi-context.py")).toBe(true);
    ...
    expect(sessionHook).toContain("write-moluoxixi-context.py session");
    expect(userHook).toContain("write-moluoxixi-context.py user");
    expect(subHook).toContain("write-moluoxixi-context.py subagent");

    const hookPy = fs.readFileSync(
      path.join(tmpDir, ".snow", "hooks", "write-moluoxixi-context.py"),
      "utf-8",
    );
    expect(hookPy).toContain("MOLUOXIXI_SNOW_HOOK_MODE");
    ...
    expect(snowGuide).not.toContain("sub-agents.moluoxixi.json");
    expect(snowGuide).toContain("Session identity");
    expect(snowGuide).toContain("SNOW_SESSION_ID");
    expect(snowGuide).toContain("MOLUOXIXI_CONTEXT_ID");
    ...
    expect(
      fs.existsSync(path.join(tmpDir, ".snow", "sub-agents.moluoxixi.json")),
    ).toBe(false);
    expect(
    ...
    expect(AI_TOOLS.snow.extraManagedPaths ?? []).not.toContain(
      ".snow/sub-agents.moluoxixi.json",
    );
  });
  ...
  it("configurePlatform('dsh') writes shared + dsh-private skills and the operator guide", async () => {
    ...
    // hasHooks=false → moluoxixi-start stays as a user-invocable dsh skill
    expect(
      fs.existsSync(
        path.join(tmpDir, ".dsh", "skills", "moluoxixi-start", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".dsh", "skills", "moluoxixi-continue", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".dsh", "skills", "moluoxixi-finish-work", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
      fs.readFileSync(
  ...
  it("configurePlatform('zcode') writes only .zcode-owned skills", async () => {
    ...
    const expectedCheck = expectedPrivateSkills.find(
      (skill) => skill.name === "moluoxixi-check",
    );
    if (!expectedCheck) {
      throw new Error("Expected ZCode private skills to include moluoxixi-check");
    }
    ...
    expect(implementAgent).toContain("Moluoxixi Context Loading Protocol");
    expect(implementAgent).toContain("<!-- moluoxixi-hook-injected -->");
    expect(implementAgent).not.toContain("Load Moluoxixi Context First");
    expect(fs.readFileSync(researchAgentPath, "utf-8")).not.toContain(
      "Load Moluoxixi Context First",
    );
    ...
    expect(templates?.has(".zcode/commands/moluoxixi/start.md")).toBe(false);
    expect(
      [...(templates?.keys() ?? [])].some((key) =>
        key.startsWith(".agents/skills/"),
      ),
    ).toBe(false);
    expect(templates?.has(".agents/skills/moluoxixi-start/SKILL.md")).toBe(false);
    expect(templates?.has(".agents/skills/moluoxixi-continue/SKILL.md")).toBe(
      false,
    );
    expect(templates?.has(".agents/skills/moluoxixi-check/SKILL.md")).toBe(false);
    expect(templates?.has(".zcode/skills/moluoxixi-start/SKILL.md")).toBe(false);
    expect(templates?.has(".zcode/skills/moluoxixi-continue/SKILL.md")).toBe(
      false,
    );
    expect(templates?.has(".zcode/skills/moluoxixi-finish-work/SKILL.md")).toBe(
      false,
    );
    expect(templates?.has(".zcode/skills/moluoxixi-check/SKILL.md")).toBe(true);
    expect(templates?.has(".zcode/skills/moluoxixi-meta/SKILL.md")).toBe(true);
    expect(templates?.has(".zcode/agents/moluoxixi-implement.md")).toBe(true);
    expect(templates?.has(".zcode/agents/moluoxixi-check.md")).toBe(true);
    expect(templates?.has(".zcode/agents/moluoxixi-research.md")).toBe(true);
    expect(templates?.get(".zcode/agents/moluoxixi-implement.md")).toContain(
      "Moluoxixi Context Loading Protocol",
    );
    expect(templates?.get(".zcode/agents/moluoxixi-implement.md")).not.toContain(
      "Load Moluoxixi Context First",
    );
    expect(templates?.get(".zcode/agents/moluoxixi-research.md")).not.toContain(
      "Load Moluoxixi Context First",
    );
  });
  ...
  it("configurePlatform('codebuddy') writes all command templates from common source", async () => {
    ...
    const commandsDir = path.join(tmpDir, ".codebuddy", "commands", "moluoxixi");
    expect(fs.existsSync(commandsDir)).toBe(true);
  ...
  it("configurePlatform('copilot') writes both tracked and discovery hooks config", async () => {
    ...
    const discovery = path.join(tmpDir, ".github", "hooks", "moluoxixi.json");
  ...
  it("configurePlatform('droid') creates .factory/commands/moluoxixi directory", async () => {
    await configurePlatform("droid", tmpDir);
    expect(
      fs.existsSync(path.join(tmpDir, ".factory", "commands", "moluoxixi")),
    ).toBe(true);
  });
  ...
  it("droid configuration writes commands + skills", async () => {
    ...
    expect(content).toContain("name: moluoxixi-check");
  });
  ...
  it("configurePlatform('pi') creates extension-backed Pi assets", async () => {
    ...
    expect(
      fs.existsSync(path.join(tmpDir, ".pi", "agents", "moluoxixi-check.md")),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(tmpDir, ".pi", "agents", "moluoxixi-research.md")),
    ).toBe(true);
    expect(
      fs.existsSync(
    ...
    expect(extension).toContain("MOLUOXIXI_PI_CLI_JS");
    expect(extension).toContain("function formatPiOutput");
    expect(extension).toContain('"## Moluoxixi Agent Definition"');
    expect(extension).toContain("ctx?.ui?.notify?.(");
    expect(extension).toContain("systemPrompt:");
    expect(extension).toContain("isMoluoxixiAgent(root, agentName)");
    expect(extension).not.toContain("message: buildMoluoxixiContext");
    expect(extension).not.toContain('message:\n      "Moluoxixi project context');
    ...
    // get_context.py is allowed: it lives in .moluoxixi/scripts/ and is the
  ...
  it("configurePlatform('pi') writes tracked templates exactly", async () => {
    ...
    for (const agent of getPiAgents()) {
      ...
      if (["moluoxixi-implement", "moluoxixi-check"].includes(agent.name)) {
  ...
  it("collectPlatformTemplates('pi') maps prompts, skills, agents, extension, and settings", () => {
    ...
    expect(templates?.get(".pi/prompts/moluoxixi-start.md")).toBeDefined();
    expect(templates?.get(".pi/prompts/moluoxixi-finish-work.md")).toBeDefined();
    expect(templates?.get(".pi/prompts/moluoxixi-continue.md")).toBeDefined();
    expect(
      templates?.get(".agents/skills/moluoxixi-check/SKILL.md"),
    ).toBeDefined();
    expect(
      templates?.get(
    ...
    expect(templates?.get(".pi/agents/moluoxixi-implement.md")).toContain(
      "Required: Load Moluoxixi Context First",
    );
    expect(templates?.get(".pi/extensions/moluoxixi/index.ts")).toBe(
  ...
  it("collectPlatformTemplates('droid') maps commands under .factory/commands/moluoxixi/", () => {
    const templates = collectPlatformTemplates("droid");
    expect(templates).toBeInstanceOf(Map);
    // Droid is agent-capable → start.md not emitted.
    expect(
      templates?.get(".factory/commands/moluoxixi/start.md"),
    ).toBeUndefined();
    expect(
      templates?.get(".factory/commands/moluoxixi/finish-work.md"),
    ).toBeDefined();
    expect(
      templates?.get(".factory/commands/moluoxixi/continue.md"),
    ).toBeDefined();
  });
  ...
  it("does not throw for any platform", async () => {
    for (const id of PLATFORM_IDS) {
      const platformDir = fs.mkdtempSync(
        path.join(os.tmpdir(), `moluoxixi-cfg-${id}-`),
      );
      try {
        setWriteMode("force");
  ...
  it("collectPlatformTemplates('copilot') includes tracked + discovery hooks config", () => {
    ...
    expect(templates?.get(".github/hooks/moluoxixi.json")).toBe(
      resolvePlaceholders(getCopilotHooksConfig()),
    );
  });
