/**
 * Execute Snow's write-moluoxixi-context.py for session / user / subagent modes.
 *
...
describe("snow write-moluoxixi-context.py execution", () => {
  ...
  function makeRepo(): string {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "moluoxixi-snow-hook-"));
    tmpRoots.push(root);
  ...
  it.skipIf(!PYTHON)(
    ...
    () => {
      ...
      const logPath = path.join(repo, ".snow", "log", "moluoxixi-context.txt");
  ...
  it.skipIf(!PYTHON)(
    ...
    () => {
      ...
      expect(res.payload?.display).toContain("Moluoxixi compact breadcrumb");

      const log = fs.readFileSync(
        path.join(repo, ".snow", "log", "moluoxixi-context.txt"),
        "utf-8",
      );
