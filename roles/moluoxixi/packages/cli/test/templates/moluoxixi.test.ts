...
describe("moluoxixi template constants", () => {
    ...
    const generatedPullBasedLabels = PLATFORM_IDS.flatMap((id) => {
      ...
      const hasPullBasedPrelude =
        ...
        [...templates.entries()].some(
          ([filePath, content]) =>
            /moluoxixi-(implement|check)/.test(filePath) &&
            content.includes("Required: Load Moluoxixi Context First"),
        );
  ...
  it("[issue-237] workflow.md in_progress breadcrumb self-exempts implement/check sub-agents", () => {
    ...
    expect(block).toContain("already running as `moluoxixi-implement`");
    expect(block).toContain("do NOT spawn another `moluoxixi-implement`");
    expect(block).toContain("already running as `moluoxixi-check`");
    expect(block).toContain("do NOT spawn another `moluoxixi-check`");
  ...
  it("[issue-237] workflow.md Phase 2 dispatch steps require prompt recursion guards", () => {
    ...
    expect(workflowMdTemplate).toContain(
      "already the `moluoxixi-implement` sub-agent",
    );
    expect(workflowMdTemplate).toContain(
      "not spawn another `moluoxixi-implement` / `moluoxixi-check`",
    );
    expect(workflowMdTemplate).toContain(
      "already the `moluoxixi-check` sub-agent",
    );
    expect(workflowMdTemplate).toContain(
      "not spawn another `moluoxixi-check` / `moluoxixi-implement`",
    );
  });
