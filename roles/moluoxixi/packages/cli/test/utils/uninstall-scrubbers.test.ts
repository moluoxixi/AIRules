...
describe("scrubHooksJson — nested schema", () => {
  ...
  it("removes the exact opt-in Moluoxixi statusLine while preserving other settings", () => {
    const input = {
      env: { USER_VALUE: "keep" },
      statusLine: {
        type: "command",
  ...
  it("preserves the Moluoxixi-shaped statusLine when the manifest does not own it", () => {
    const input = {
      statusLine: {
        type: "command",
        command: "python3 .claude/hooks/statusline.py",
...
`;
  ...
  it("preserves user-owned keys in the agents table while removing Moluoxixi max_depth", () => {
    ...
    expect(content).not.toContain("Moluoxixi");
  });
