 ...
 * `moluoxixi channel spawn --agent <name>`, which loads `.moluoxixi/agents/<name>.md`
 ...
 * runtime. We surface that mismatch eagerly (at `moluoxixi init --workflow` /
 * `moluoxixi workflow --template` time) so users can run `moluoxixi update` before
 * the first spawn.
 *
 ...
 *
 *   1. `--agent <name>` flag on a `moluoxixi channel spawn ...` command
 *   2. `.moluoxixi/agents/<name>.md` literal path reference
 *
...
const AGENT_PATH_RE = new RegExp(
  `\\.moluoxixi/agents/([${SAFE_AGENT_NAME_CHARS}]+)\\.md`,
  "g",
);

/**
 * Extract the set of `.moluoxixi/agents/<name>.md` agent names that the given
 ...
 * exist under `<cwd>/.moluoxixi/agents/`.
 */
