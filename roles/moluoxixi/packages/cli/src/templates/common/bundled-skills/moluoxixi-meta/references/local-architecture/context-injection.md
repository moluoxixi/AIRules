...
Moluoxixi context injection aims to make AI read the right files at the right time instead of relying on model memory. In a user project, injection is implemented by `.moluoxixi/` scripts together with platform hooks, agents, and skills.

## Injected Context Types

| Type | Source | Purpose |
| --- | --- | --- |
| session context | `.moluoxixi/scripts/get_context.py` | Current developer, git status, active task, active tasks, journal, packages. |
| workflow context | `.moluoxixi/workflow.md` | Current Moluoxixi flow and next action. |
| spec context | `.moluoxixi/spec/` + task JSONL | Specs that must be followed during implementation/checking. |
| task context | `.moluoxixi/tasks/<task>/prd.md`, `design.md`, `implement.md`, `research/` | Current task requirements, design, execution plan, and research. |
...
Platforms with session-start support inject a Moluoxixi overview when a session starts, clears, compacts, or receives a similar event. Injected content usually includes:
...
workflow-state is a lightweight hint injected around each user turn. Based on current task status, it selects a block from `.moluoxixi/workflow.md`, such as `no_task`, `planning`, `in_progress`, or `completed`.

If the user wants to change "what the AI should do next in a given state," edit the corresponding state block in `.moluoxixi/workflow.md` first.

## sub-agent context

Implement and check agents need task context. Moluoxixi has two loading modes:
...
```jsonl
{"file": ".moluoxixi/spec/backend/index.md", "reason": "Backend rules"}
```
...
| Change per-turn workflow-state rules | `[workflow-state:STATUS]` block in `.moluoxixi/workflow.md`. The platform workflow-state hook parses these blocks verbatim and embeds no fallback text. |
| Change how sub-agents read context | Platform agent definitions, the `inject-subagent-context` hook, or agent preludes. |
| Change JSONL validation/display | `.moluoxixi/scripts/common/task_context.py`. |
| Change active task resolution | `.moluoxixi/scripts/common/active_task.py`. |
