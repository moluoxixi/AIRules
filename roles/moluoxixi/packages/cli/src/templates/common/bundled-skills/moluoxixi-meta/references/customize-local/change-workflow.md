...
When the user wants to change Moluoxixi phases, next-action hints, whether to create tasks, whether to use sub-agents, or when to check/wrap up, edit `.moluoxixi/workflow.md` first.
...
1. Find the relevant section in `.moluoxixi/workflow.md`.
...
4. Workflow-state changes only need an edit to the `[workflow-state:STATUS]` block in `.moluoxixi/workflow.md`. The hook is parser-only — it reads whatever you put in the block. Keep the opening and closing tags' STATUS strings identical (`[workflow-state:foo]…[/workflow-state:foo]`); mismatched STATUS pairs are silently dropped.
5. Make the AI reread `.moluoxixi/workflow.md`; do not keep using rules from the old conversation.
...
| --- | --- | --- |
| `planning` | `prd.md` missing | Phase 1.1 (load `moluoxixi-brainstorm`) |
...
`.moluoxixi/workflow.md` is the local project workflow, not an immutable template. The user can adapt it to team habits. After editing it, platform entry files may still contain old descriptions, so inspect them too.
