...
| --------------------------------------------- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--platform claude\|codex\|opencode\|pi\|all` | all               | Default `all`. OpenCode sessions are read from the local SQLite store.                                                                                 |
...
- **OpenCode sessions are read-only.** The adapter snapshots and parses the local WAL-mode SQLite store; unreadable or unsupported databases produce a structured warning and an empty result rather than modifying the store.
...
- **`mem` indexes local platform session stores directly.** If the user has cleared their Claude / Codex / OpenCode / Pi session storage, `mem` cannot recover what is no longer on disk.
- **`mem` is read-only.** No remote sync and no edits to platform stores. Any write you do based on `mem` findings is your own follow-up call into the editing tools available to you.
