...
| `templates/common/commands/<name>.md` | Slash command / prompt | Written to each platform's command directory (`.claude/commands/trellis/`, `.cursor/commands/moluoxixi-*.md`, `.gemini/commands/trellis/*.toml`, etc.) |
...
| `moluoxixi-session-insight` | Wraps the `trellis mem` CLI so an AI knows when and how to reach into past Claude Code / Codex / Pi Agent conversation logs. |
...
| `moluoxixi-channel` | Capability skill teaching an AI when to reach for `trellis channel` for multi-agent collaboration, forum/thread persistent boards, and dispatcher-wait patterns. |
...
2. `trellis update` → `collectPlatformTemplates(platformId)` (in `configurators/index.ts`) → the same map, used to detect drift and to populate `.moluoxixi/.template-hashes.json`.
...
3. The next `trellis update` detects the user modification and leaves the file untouched (Moluoxixi never overwrites user-modified files without an explicit `--force`).
