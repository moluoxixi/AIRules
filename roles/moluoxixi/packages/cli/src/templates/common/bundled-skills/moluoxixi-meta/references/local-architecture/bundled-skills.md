...
| Skill | Purpose |
| --- | --- |
| `moluoxixi-meta` | This skill. Explains the local Moluoxixi architecture and customization entry points to an AI working inside a user project. |
| `moluoxixi-session-insight` | Wraps the `moluoxixi mem` CLI so an AI knows when and how to reach into past Claude Code / Codex / Pi Agent conversation logs. |
| `moluoxixi-spec-bootstrap` | Platform-neutral workflow for creating or refreshing `.moluoxixi/spec/` from the real codebase (with optional GitNexus / ABCoder integration). |
| `moluoxixi-channel` | Capability skill teaching an AI when to reach for `moluoxixi channel` for multi-agent collaboration, forum/thread persistent boards, and dispatcher-wait patterns. |
...
1. `moluoxixi init` → `configurePlatform(platformId, cwd)` → `writeTemplateMap(cwd, collect<Platform>Templates())`. For 18 of the 21 platforms the registry entry in `configurators/index.ts` is literally `fromTemplates(collect<Platform>Templates)`, which *is* that composition. Claude Code, Codex and ZCode spell out a `configure` of their own, each for work a `Map<path, content>` cannot express (an opt-in `--with-statusline` flag, an intentionally empty `.codex/skills/` directory, a one-shot console notice) — none of them restates the file list.
2. `moluoxixi update` → `collectPlatformTemplates(platformId)` (in `configurators/index.ts`) → the same map, used to detect drift and to populate `.moluoxixi/.template-hashes.json`.
...
5. **Verify the distribution path** before shipping. Skipping any of these steps has historically caused features to be documented as bundled while the published npm tarball was missing the files:
   ...
   - `pnpm --filter @moluoxixi/airules-moluoxixi build` copies the asset into `dist/templates/common/bundled-skills/<skill>/`.
...
1. Edit the local file directly. Example: `.claude/skills/moluoxixi-meta/SKILL.md`.
...
- Team-private conventions belong in `.moluoxixi/spec/` or in a separate marketplace-style local skill, not in modifications to `moluoxixi-meta` itself. See `customize-local/add-project-local-conventions.md`.
