---
description: "Moluoxixi Copilot prompt: Multi-Agent Pipeline Orchestrator"
---
...
### Step 1: Understand Moluoxixi Workflow `[AI]`

First, read the workflow guide to understand the development process:

```bash
cat .moluoxixi/workflow.md  # Development process, conventions, and quick start guide
```
...
```bash
python3 ./.moluoxixi/scripts/get_context.py --mode packages  # Discover available spec layers
cat .moluoxixi/spec/guides/index.md    # Thinking guides
```
...
```bash
python3 ./.moluoxixi/scripts/multi_agent/plan.py \
  --name "<feature-name>" \
...
```bash
python3 ./.moluoxixi/scripts/multi_agent/start.py "$TASK_DIR"
```
...
python3 ./.moluoxixi/scripts/task.py init-context "$TASK_DIR" <dev_type>

# Set branch and scope
python3 ./.moluoxixi/scripts/task.py set-branch "$TASK_DIR" feature/<name>
python3 ./.moluoxixi/scripts/task.py set-scope "$TASK_DIR" <scope>
```
...
```bash
python3 ./.moluoxixi/scripts/task.py validate "$TASK_DIR"
python3 ./.moluoxixi/scripts/multi_agent/start.py "$TASK_DIR"
```
...
```bash
python3 ./.moluoxixi/scripts/multi_agent/status.py                    # Overview
python3 ./.moluoxixi/scripts/multi_agent/status.py --log <name>       # View log
python3 ./.moluoxixi/scripts/multi_agent/status.py --watch <name>     # Real-time monitoring
python3 ./.moluoxixi/scripts/multi_agent/cleanup.py <branch>          # Cleanup worktree
```
