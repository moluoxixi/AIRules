...
def _read_file_bytes(base_path: str, file_path: str) -> bytes | None:
    ...
    try:
        ...
        # `.moluoxixi` may itself be a symlink into a store outside the repo
        # (#567); its real location is a second legitimate containment base.
        workflow_real = os.path.realpath(os.path.join(base_path, ".moluoxixi"))
...
def _binary_notice(path: str, size: int, reason: str) -> str:
    return (
        f"[Moluoxixi: not inlined (binary file) — "
...
def read_jsonl_entries(base_path: str, jsonl_path: str) -> list[dict]:
    ...
    Rows without a ``file`` field (e.g. the placeholder line older Moluoxixi
...
def get_agent_context(
    ...
    if not blocks:
        ...
        return (
            f"[Moluoxixi] {agent_jsonl} has no curated entries, so no spec/research "
            "context was injected. Before working, read the guidelines relevant "
            "to the code you will touch under .moluoxixi/spec/, and treat the task "
...
def _codex_subagent_type(input_data: dict) -> str:
    """Return a Moluoxixi Codex agent type only for a native start event."""
    if _hook_event_name(input_data) != "SubagentStart":
        return ""
...
spawn another Moluoxixi subagent.
...
def _handle_codex_subagent_start(input_data: dict) -> None:
    """Emit Codex developer context for a recognised native Moluoxixi subagent.
...
def main():
    ...
    if subagent_type in AGENTS_REQUIRE_TASK:
        ...
        try:
            ...
            # `.moluoxixi` may itself be a symlink into a store outside the
            # repo (#567); its real location is a second legitimate base.
            workflow_real = os.path.realpath(os.path.join(repo_root, ".moluoxixi"))
