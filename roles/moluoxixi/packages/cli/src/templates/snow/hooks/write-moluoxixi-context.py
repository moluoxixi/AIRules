...
def _parse_active_task_path(current_out: str, repo: Path) -> Path | None:
    # Prefer "Current task: .moluoxixi/tasks/..." line from task.py --source.
    ...
    m = re.search(r"(\.moluoxixi[/\\]tasks[/\\][^\s]+)", current_out)
    if m:
...
def _resolve_runtime_session_file(
    ...
    sessions_dir = repo / ".moluoxixi" / ".runtime" / "sessions"
...
def _workflow_phase_summary(
    ...
    lines: list[str] = []
    workflow = repo / ".moluoxixi" / "workflow.md"
    if workflow.is_file():
    ...
    session_md = repo / ".moluoxixi" / "session" / "current.md"
    if session_md.is_file():
        body = _read_text(session_md, 1200)
        if body:
            lines.extend(["## .moluoxixi/session/current.md", body, ""])
    else:
...
def build_context(
    ...
    *,
    ...
    task_py = repo / ".moluoxixi" / "scripts" / "task.py"
    current = ""
    ...
    if not compact:
        ...
        identity = repo / ".moluoxixi" / "identity.md"
        if identity.is_file():
            body = _read_text(identity, 900 if mode == "subagent" else 1200)
            if body:
                lines.extend(["## .moluoxixi/identity.md", body, ""])
    else:
    ...
    if mode == "subagent" and kind == "research":
        ...
        lines.extend(
            [
                ...
                "1. Session/user hooks auto-inject Moluoxixi context; `/moluoxixi-start` is optional."
                if not compact
                else "1. Compact inject — full context also at `.snow/log/moluoxixi-context.txt`.",
                "2. When dispatching implement/check/research, start the prompt with:",
                "   `Active task: <path from task.py current>`",
                "3. Do not git commit/push/merge from Moluoxixi implement/check agents.",
                "",
            ]
        )
...
def main() -> int:
    try:
        ...
        try:
            ...
            # Keep moluoxixi-context.txt as the richest practical breadcrumb for
            # pull-based reads. User-mode inject stays compact, but must not
            # clobber a prior full snapshot.
            log_path = log_dir / "moluoxixi-context.txt"
            if mode == "user":
            ...
            sys.stderr.write(f"moluoxixi-context write failed: {exc}\n")

        max_bytes = COMPACT_MAX_BYTES if mode == "user" else DEFAULT_MAX_BYTES
        inject = _truncate(context, max_bytes)
        display = {
            "session": "Moluoxixi session context injected",
            "user": "Moluoxixi compact breadcrumb",
            "subagent": f"Moluoxixi sub-agent context ({_agent_kind(stdin_ctx)})",
        }.get(mode, "Moluoxixi context refreshed")
        display = f"{display} (.snow/log/moluoxixi-context.txt)"
        ...
        try:
            sys.stderr.write(f"write-moluoxixi-context failed: {exc}\n")
            # Still emit empty-safe JSON so host can continue.
            sys.stdout.write(
                json.dumps(
                    {
                        "additionalContext": "# Moluoxixi context unavailable (hook error)\n",
                        "display": "Moluoxixi context hook error (fail-open)",
                    },
                    ensure_ascii=False,
                )
            )
