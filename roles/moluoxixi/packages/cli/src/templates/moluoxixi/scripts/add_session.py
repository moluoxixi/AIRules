...
MARKER_PREFIX = "<!-- moluoxixi-session:"
...
MARKER_VERSION = 2
LEGACY_MARKER_RE = re.compile(r"^<!-- moluoxixi-session: fp=([0-9a-f]{16}) -->$")
...
def warn_if_parallel_worktree(repo_root: Path) -> None:
    ...
    print(
        colored(
            ...
            "not index.md). See .moluoxixi/spec/cli/backend/directory-structure.md "
...
def content_at_head(repo_root: Path, path: Path) -> str | None:
    ...
    Moluoxixi root is the nearest directory holding `.moluoxixi/`, which can sit
...
def _auto_commit_workspace(repo_root: Path) -> str:
    ...
    ``COMMIT_FAILED``. ``COMMIT_BLOCKED`` is the gitignored-`.moluoxixi/` case:
