...
Note:
    ...
    ``.moluoxixi/workflow.md`` for the current planning artifact contract.

    Older Moluoxixi versions seeded those files with a ``{"_example": ...}``
...
# one of these extensions outside .moluoxixi/spec/, docs/docs-site, or the
...
def _is_exempt_from_code_file_warning(file_path: str, task_rel: str) -> bool:
    ...
    Exempt: spec docs (``.moluoxixi/spec/``), documentation (``docs``,
...
def _validate_jsonl(jsonl_file: Path, repo_root: Path, task_dir: Path | None = None) -> int:
    ...
    ``{"_example": ...}`` placeholder rows written by older Moluoxixi versions
    ...
    if errors == 0 and real_entries == 0:
        ...
        print(
            f"    Curate it:  python3 .moluoxixi/scripts/task.py add-context <task> {action} <path> \"<why>\""
        )
        print(
