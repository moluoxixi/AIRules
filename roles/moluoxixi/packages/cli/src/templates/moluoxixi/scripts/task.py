...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "allow_empty_context", False):
        ...
        if empty_manifests:
            ...
            print(f"  Curate:  python3 .moluoxixi/scripts/task.py add-context {task_input} implement <path> \"<why>\"")
            print(f"  Verify:  python3 .moluoxixi/scripts/task.py validate {task_input}")
...
Examples:
  ...
  python3 task.py create "Child task" --description "Session cookie handling" --slug child --parent .moluoxixi/tasks/01-21-parent
