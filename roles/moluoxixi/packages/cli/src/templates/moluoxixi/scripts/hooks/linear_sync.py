#!/usr/bin/env python3
"""Linear sync hook for Moluoxixi task lifecycle.

Syncs task events to Linear via the `linearis` CLI.

Usage (called automatically by task.py hooks):
    python3 .moluoxixi/scripts/hooks/linear_sync.py create
    python3 .moluoxixi/scripts/hooks/linear_sync.py start
    python3 .moluoxixi/scripts/hooks/linear_sync.py archive

Manual usage:
    TASK_JSON_PATH=.moluoxixi/tasks/<name>/task.json python3 .moluoxixi/scripts/hooks/linear_sync.py sync

Environment:
    TASK_JSON_PATH  - Absolute path to task.json (set by task.py)

Configuration:
    .moluoxixi/hooks.local.json  - Local config (gitignored), example:
    {
      "linear": {
        "team": "TEAM_KEY",
...
# Moluoxixi priority → Linear priority (1=Urgent, 2=High, 3=Medium, 4=Low)
...
def _load_config() -> dict:
    """Load local hook config from .moluoxixi/hooks.local.json."""
