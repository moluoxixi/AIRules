...
class CLIAdapter:
    ...
    _AGENT_NAME_MAP: ClassVar[dict[Platform, dict[str, str]]] = {
        ...
        "opencode": {
            "plan": "moluoxixi-plan",  # 'plan' is built-in in OpenCode
        },
    }
    ...
    def get_agent_name(self, agent: str) -> str:
        ...
        Returns:
            Platform-specific agent name (e.g., 'moluoxixi-plan' for OpenCode)
        """
    ...
    def get_commands_path(self, project_root: Path, *parts: str) -> Path:
        ...
        Note:
            Cursor uses prefix naming: .cursor/commands/moluoxixi-<name>.md
        ...
        if self.platform == "pi":
            ...
            if len(parts) >= 2 and parts[0] == "trellis":
                ...
                if filename.endswith(".md"):
                    filename = filename[:-3]
                return prompts_dir / f"moluoxixi-{filename}.md"
            return prompts_dir / Path(*parts)
        # OMP and Grok: flat slash commands under .{platform}/commands/moluoxixi-<name>.md
        if self.platform in ("omp", "grok"):
            ...
            if len(parts) >= 2 and parts[0] == "trellis":
                ...
                if filename.endswith(".md"):
                    filename = filename[:-3]
                return commands_dir / f"moluoxixi-{filename}.md"
            return commands_dir / Path(*parts)

        # Kimi: commands are skills under .kimi-code/skills/moluoxixi-<name>/SKILL.md
        if self.platform == "kimi":
            ...
            if len(parts) >= 2 and parts[0] == "trellis":
                ...
                if filename.endswith(".md"):
                    filename = filename[:-3]
                return skills_dir / f"moluoxixi-{filename}" / "SKILL.md"
    ...
    def get_trellis_command_path(self, name: str) -> str:
        ...
        if self.platform == "cursor":
            ...
            # 0.5.0-beta.0 renamed all skill dirs to add the `moluoxixi-` prefix
            # (see that release's manifest for the 60+ rename entries).
            return f".agents/skills/moluoxixi-{name}/SKILL.md"
        elif self.platform == "kiro":
            ...
            return f".devin/workflows/moluoxixi-{name}.md"
        elif self.platform == "kilo":
...
def detect_platform(project_root: Path) -> Platform:
    ...
    # (.agents/skills/moluoxixi-* dirs) and no explicit platform config dir.
    ...
    if agents_skills.is_dir() and not _has_other_platform_dir(
        ...
        try:
            for entry in agents_skills.iterdir():
                if entry.is_dir() and entry.name.startswith("moluoxixi-"):
                    return "codex"
        except OSError:
            pass
