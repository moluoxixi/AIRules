...
def is_within_tasks_dir(task_dir_abs: Path, repo_root: Path | None = None) -> bool:
    ...
    A real task lives directly at ``.moluoxixi/tasks/<name>``. This returns True
...
def resolve_task_dir(target_dir: str, repo_root: Path) -> Path | None:
    ...
    - Relative path: .moluoxixi/tasks/01-31-my-task
    ...
    land strictly under ``.moluoxixi/tasks/``; archived tasks under
    ...
    ``.moluoxixi`` that is itself a symlink into a shared store (#567) works.
    ...
    elif "/" in normalized or normalized.startswith(".moluoxixi"):
        # Relative path (contains path separator or starts with .moluoxixi)
    ...
    # `resolved`: `.moluoxixi` may be a symlink into a store outside the repo
...
def run_task_hooks(event: str, task_json_path: Path, repo_root: Path) -> None:
    ...
    Hooks are shell commands read from ``.moluoxixi/config.yaml`` and executed
    with ``shell=True`` from the repo root — see the trust boundary note in
    ``.moluoxixi/spec/cli/backend/script-conventions.md``.
...
if __name__ == "__main__":
    ...
    print(f"resolve_task_dir('.moluoxixi/tasks/test'): {resolve_task_dir('.moluoxixi/tasks/test', repo)}")
