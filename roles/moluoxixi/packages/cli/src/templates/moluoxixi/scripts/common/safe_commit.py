"""
Safe git-add helpers for Moluoxixi-owned paths.

Why this module exists
----------------------
A real user incident: a project's `.gitignore` listed `.moluoxixi/` (company-wide
...
- Scripts only stage SPECIFIC product paths (journal files, index.md, the
  current task dir, the archive dir). Never the whole `.moluoxixi/` tree.
- If plain `git add <specific>` fails with "ignored by", DO NOT retry with
  ``-f``. The presence of `.moluoxixi/` in `.gitignore` is treated as user
  intent ("keep .moluoxixi/ local-only"). The script warns and skips the
...
- The warning includes a negative example: ``Do NOT use `git add -f .moluoxixi/` ...``
...
def safe_trellis_paths_to_add(
    ...
    Included:
      - .moluoxixi/workspace/<developer>/journal-*.md
      - .moluoxixi/workspace/<developer>/index.md
      - .moluoxixi/tasks/<task_name>/   (ONLY the current task dir when
...
def safe_archive_paths_to_add(
    ...
    `.moluoxixi/tasks/` subtree the old way (active tasks + archive). New
...
def print_gitignore_warning(paths: list[str]) -> None:
    ...
    ``Do NOT use `git add -f .moluoxixi/``` — agents reading the warning are
    known to invent that command, which fans out to ignored caches/backups.
    """
    print(
        "[WARN] git add failed because .moluoxixi/ paths are ignored by your .gitignore.",
        file=sys.stderr,
    )
    print(
    ...
    print(
        "[WARN] Moluoxixi manages these specific paths and they should be tracked:",
        file=sys.stderr,
    )
    if paths:
        for p in paths:
        ...
        print(
            "[WARN]   .moluoxixi/workspace/<developer>/{journal-*.md,index.md}",
            file=sys.stderr,
        )
        print(
            "[WARN]   .moluoxixi/tasks/<task-dir>/",
            file=sys.stderr,
        )
        print(
            "[WARN]   .moluoxixi/tasks/archive/",
            file=sys.stderr,
        )
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Recommended: change your .gitignore from `.moluoxixi/` to specific",
        file=sys.stderr,
    )
    print(
    ...
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Or, if you intentionally keep .moluoxixi/ local-only, set in",
        file=sys.stderr,
    )
    print(
        "[WARN] .moluoxixi/config.yaml:",
        file=sys.stderr,
    )
    print(
    ...
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Do NOT use `git add -f .moluoxixi/` — it pulls in backups, worktrees,",
        file=sys.stderr,
    )
    print(
