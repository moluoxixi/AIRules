#!/usr/bin/env python3
"""
Moluoxixi configuration reader.

Reads settings from .moluoxixi/config.yaml with sensible defaults.
"""
...
# The YAML subset parser lives in moluoxixi_config.py — it imports nothing from
...
def _load_config(repo_root: Path | None = None) -> dict:
    ...
    Fail-open, matching ``moluoxixi_config.read_moluoxixi_config``: a malformed
...
def get_session_auto_commit(repo_root: Path | None = None) -> bool:
    ...
    Set ``session_auto_commit: false`` in ``.moluoxixi/config.yaml`` to skip
...
def get_codex_dispatch_mode(repo_root: Path | None = None) -> str:
    ...
    Default is ``auto``, which dispatches Moluoxixi sub-agents and uses native
...
def get_context_injection_limits(repo_root: Path | None = None) -> dict[str, int]:
    ...
    Reads the ``context_injection:`` section of ``.moluoxixi/config.yaml``:
...
def get_spec_base(package: str | None = None, repo_root: Path | None = None) -> str:
    """Get the spec directory base path relative to .moluoxixi/.
