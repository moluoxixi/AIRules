#!/usr/bin/env python3
"""
Common path utilities for Moluoxixi workflow.
...
ENV_DEVELOPER = "MOLUOXIXI_DEVELOPER"
...
def get_repo_root(start_path: Path | None = None) -> Path:
    """Find the nearest directory containing .moluoxixi/ folder.
    ...
    Returns:
        Path to repository root, or current directory if no .moluoxixi/ found.
    """
    ...
    # Fallback to current directory if no .moluoxixi/ found
...
def get_developer(repo_root: Path | None = None) -> str | None:
    ...
    it, before this function is ever called):
        ...
        1. The ``MOLUOXIXI_DEVELOPER`` environment variable.
        2. ``.moluoxixi/.developer`` in this checkout.
        3. ``.moluoxixi/.developer`` in the main checkout, when this checkout is a
           linked git worktree.
...
def normalize_task_ref(task_ref: str) -> str:
    ...
    `.moluoxixi/tasks/03-27-my-task`, even on Windows. Absolute paths are preserved
...
def resolve_task_ref(task_ref: str, repo_root: Path | None = None) -> Path | None:
    ...
    trips through the session pointer under `.moluoxixi/.runtime/sessions/`, and
    `..` segments used to survive that trip intact: `_canonical_task_ref`
    compares lexically, and a lexical `relative_to` accepts
    `<root>/.moluoxixi/tasks/../../../elsewhere` because the string does start
    with the root. The ref was then stored verbatim and replayed on every later
    turn, so `task.py start .moluoxixi/tasks/../../../elsewhere` both rewrote that
    ...
    # `.moluoxixi` may itself be a symlink into a store outside the repo (#567).
...
def get_spec_dir(package: str | None = None, repo_root: Path | None = None) -> Path:
    ...
    Single-repo: .moluoxixi/spec
    Monorepo with package: .moluoxixi/spec/<package>
