...
def cmd_create(args: argparse.Namespace) -> int:
    ...
    # moluoxixi-before-dev skill instead of JSONL.
    created_jsonl = False
    ...
    if created_jsonl:
        ...
        print(
            "      list available specs: python3 .moluoxixi/scripts/get_context.py --mode packages",
            file=sys.stderr,
        )
    print("  - Use /moluoxixi:continue or phase context to decide the next step", file=sys.stderr)
    print("", file=sys.stderr)
...
class _RenamePlan:
    ...
    # (file, line number) of old-name mentions elsewhere under .moluoxixi/
...
def _plan_reported_refs(
    ...
    """Find remaining mentions of the old task name elsewhere under .moluoxixi/.
    ...
    *are* rewritten — see ``repoint_task_in_sessions``. Listing them here as
    ...
    moluoxixi_dir = repo_root / DIR_WORKFLOW
    sessions_dir = _runtime_sessions_dir(repo_root)
    if not moluoxixi_dir.is_dir():
        return hits

    for path in sorted(moluoxixi_dir.rglob("*")):
        ...
        if any(
            ...
            for part in path.relative_to(moluoxixi_dir).parts[:-1]
        ):
            continue
        try:
...
def cmd_archive(args: argparse.Namespace) -> int:
    ...
    # .moluoxixi/tasks/. resolve_task_dir keeps the target inside the tasks dir;
    # this narrows it further to a direct child, so an already-archived task
    # (.moluoxixi/tasks/archive/<month>/<task>) is not archived a second time.
    if not is_within_tasks_dir(task_dir, repo_root):
        print(colored(
            f"Error: refusing to archive '{task_name}': "
                    ...
                    if child_dir_path:
                        ...
                        if child_json.is_file():
                            ...
                            if not write_json(child_json, child_data):
                                ...
                                # parent that has left .moluoxixi/tasks/ is a
...
def _auto_commit_archive(
    ...
    """Stage Moluoxixi-owned task paths and commit after archive.
    ...
    ``git add -f .moluoxixi/`` (which would fan out to caches/backups)
    and points users at ``session_auto_commit: false``.

    Honors ``session_auto_commit`` in ``.moluoxixi/config.yaml``: when
...
def _print_index_lock_warning(
    ...
    command keeps the same narrow scope — a blanket ``git add -A -- .moluoxixi/``
