...
function binaryNotice(path, size, reason) {
  return `[Moluoxixi: not inlined (binary file) — ${path} (${size} bytes): ${reason}]`
}

function indexNotice(path, size, reason) {
  return `[Moluoxixi: not inlined (total context limit reached) — ${path} (${size} bytes): ${reason}]`
}
...
export class MoluoxixiContext {
  constructor(directory) {
    ...
    debugLog("context", "MoluoxixiContext initialized", { directory })
  }

  // ============================================================
  // Moluoxixi Project Detection
  // ============================================================

  isMoluoxixiProject() {
    return existsSync(join(this.directory, ".moluoxixi"))
  }
  ...
  readContext(contextKey) {
    try {
      const contextPath = join(this.directory, ".moluoxixi", ".runtime", "sessions", `${contextKey}.json`)
  ...
  _resolveSingleSessionFallback() {
    const sessionsDir = join(this.directory, ".moluoxixi", ".runtime", "sessions")
  ...
  resolveTaskDir(taskRef) {
    ...
    const candidate = isAbsolute(normalized)
      ...
      : normalized.startsWith(".moluoxixi/")
        ...
        : join(this.directory, ".moluoxixi", "tasks", normalized)
