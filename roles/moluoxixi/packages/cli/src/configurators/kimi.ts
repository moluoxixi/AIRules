 ...
 *   commands as skills (`/skill:moluoxixi-start`, `/skill:moluoxixi-continue`,
 *   `/skill:moluoxixi-finish-work`) plus the Moluoxixi agent prompts
 *   (moluoxixi-implement / moluoxixi-check / moluoxixi-research) with the
 *   pull-based prelude on implement/check.
 * - `.kimi-code/agents/` — the same Moluoxixi agent prompts as project-level
 *   custom sub-agent definitions (Claude Code-compatible frontmatter), so
 *   the main session can dispatch `moluoxixi-<name>` sub-agents directly.
 *
 * Moluoxixi does not generate project-local Kimi settings or hooks.
 ...
 * (`/skill:moluoxixi-<name>`). Kimi has no slash-command mechanism besides
...
/** Moluoxixi agent prompts as Kimi skills (and `.kimi-code/agents/` sub-agent
...
export function collectKimiTemplates(): Map<string, string> {
  ...
  // 2. Commands-as-skills + Moluoxixi agent prompts → `.kimi-code/skills/`.
  ...
  //    included, so dispatching `moluoxixi-<name>` directly behaves the same.
