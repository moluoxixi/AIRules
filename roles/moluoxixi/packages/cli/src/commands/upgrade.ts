...
function binaryCheckCommand(
  ...
  return platform === "win32" ? "where moluoxixi" : "which moluoxixi";
}
...
function troubleshooting(plan: UpgradeCommandPlan): string {
  return [
    ...
    `- Check which Moluoxixi binary your shell resolves: ${plan.binaryCheckCommand}`,
    "- If this is a permissions error, fix your Node/npm install or npm prefix; Moluoxixi does not run sudo.",
    "- If npm reports an existing binary or locked file, resolve that npm error manually; Moluoxixi does not run --force.",
  ].join("\n");
}
...
export async function upgrade(
  ...
  console.log(chalk.cyan(`Upgrading Moluoxixi CLI to ${plan.target}`));
  ...
  console.log(chalk.green("\n✓ Moluoxixi CLI upgrade completed"));
  console.log(chalk.gray("Run: moluoxixi --version"));
