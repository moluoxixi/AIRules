...
```bash
python3 ./.moluoxixi/scripts/get_context.py --mode record
```
...
Filter out paths under `.moluoxixi/workspace/` and `.moluoxixi/tasks/` — those are managed by `add_session.py` and `task.py archive` auto-commits and will appear dirty as part of this skill's own work.
...
- **Any remaining path looks like current-task work** — bail out with:
  > "Working tree has uncommitted code changes from this task: `<list>`. Return to workflow Phase 3.4 to commit them before running `/moluoxixi:finish-work`."
...
```bash
python3 ./.moluoxixi/scripts/task.py archive <task-name>
```
...
```bash
python3 ./.moluoxixi/scripts/add_session.py \
  --title "Session Title" \
  --commit "hash1,hash2" \
