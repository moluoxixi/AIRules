...
```jsonl
{"file": ".moluoxixi/spec/cli/backend/index.md", "reason": "Backend guidelines"}
{"file": "src/services/auth.ts", "reason": "Existing auth service"}
{"file": ".moluoxixi/tasks/01-31-add-login/prd.md", "reason": "Requirements"}
```
...
### `.moluoxixi/.runtime/sessions/<session-key>.json`

Stores the active task for one AI session/window.

```json
{
  "current_task": ".moluoxixi/tasks/01-31-add-login"
}
```
...
```bash
python3 .moluoxixi/scripts/task.py create "Task name" --slug task-slug
python3 .moluoxixi/scripts/task.py create "Child task" --slug child --parent <parent-dir>
```
...
```bash
python3 .moluoxixi/scripts/task.py list
python3 .moluoxixi/scripts/task.py list --mine
python3 .moluoxixi/scripts/task.py list --status planning
```
...
```bash
python3 .moluoxixi/scripts/task.py init-context <task-dir> <dev-type>
```
...
```bash
python3 .moluoxixi/scripts/task.py archive <task-dir>
```
...
```bash
python3 .moluoxixi/scripts/task.py set-branch <dir> <branch>
python3 .moluoxixi/scripts/task.py set-base-branch <dir> <branch>
python3 .moluoxixi/scripts/task.py set-scope <dir> <scope>
python3 .moluoxixi/scripts/task.py add-context <dir> <jsonl> <path> [reason]
python3 .moluoxixi/scripts/task.py validate <dir>
python3 .moluoxixi/scripts/task.py list-context <dir>
python3 .moluoxixi/scripts/task.py list-archive [month]
python3 .moluoxixi/scripts/task.py create-pr [dir] [--dry-run]
```
...
```bash
python3 .moluoxixi/scripts/get_context.py                      # Default text (full context)
python3 .moluoxixi/scripts/get_context.py --json                # Default JSON
python3 .moluoxixi/scripts/get_context.py --mode record         # Record text (my tasks focus)
python3 .moluoxixi/scripts/get_context.py --mode record --json  # Record JSON
```
