...
- **当前任务 Markdown**（`writing-plans` 产出的单任务文件，路径为 `.airules/tasks/<task-name>.md`）：coder 以单个任务为最小工作单位，每次只消费一个任务文件，不一次消费整个计划
- **该任务对应的测试用例文件**（`test-design` 落档在 `.airules/tests/<feature>.md` 的验收清单，或任务文件内嵌的 AC-id 映射）：执行前读取相关 TC-id / AC-id，作为 TDD 红阶段的测试设计依据
- 目标文件范围（来自任务 Markdown 的"涉及文件与职责"）
- 禁止事项（不可触碰的范围 / 行为）
