...
最小化项目初始化：注入项目规则、建立 `CLAUDE.md` 链接、初始化 CodeGraph、运行 OpenSpec 项目初始化，并把 `superpowers-bridge` schema 注入到项目级 `openspec/schemas/superpowers-bridge/`。

## 触发条件

- 用户创建新项目、初始化项目，或首次为已有项目接入 AIRules。
- 需要生成/补充项目根 `AGENTS.md`、建立 `CLAUDE.md` 链接、初始化 CodeGraph 或注册 OpenSpec schema。
...
- 只改初始化交付物：项目根 `AGENTS.md`、`CLAUDE.md` 链接、CodeGraph 初始化结果、`openspec/schemas/superpowers-bridge/**` 与 `knowledge/index.md`。
...
flowchart TD
  ...
  F --> G[spec-init.mjs 运行 openspec init 并注入 schema]
  G --> H[交付检查]
```
...
| OpenSpec schema | `node "<init-project-skill>/scripts/spec-init.mjs" <project>` | 若项目缺 `openspec/`，先运行 `openspec init <project> --tools none --no-color`；随后从 `assets/superpowers-bridge/` 与 `assets/knowledge/index.md` 复制缺失文件到 `openspec/schemas/superpowers-bridge/**` 与 `knowledge/index.md`，并运行 `openspec schema validate superpowers-bridge` 与 `openspec schemas` 确认项目级注册 | 幂等，已存在文件不覆盖；`openspec` 命令缺失时标 `MISSING` 并失败，不用本地骨架伪装完整初始化 |
...
| OpenSpec schema | `openspec/` 由 OpenSpec CLI 初始化；`openspec/schemas/superpowers-bridge/` 存在；`openspec schema validate superpowers-bridge` 通过；`openspec schemas` 能列出 `superpowers-bridge (project)` 或等价项目级条目 |
