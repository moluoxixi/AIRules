...
2. Treat `~/.codex/rules/` as the first-party stack guidance layer.
3. Treat first-party and vendored skills exposed through `~/.agents/skills/` as task-specific depth.
...
- Edit first-party rules, skills, agents, and docs in the `aiRules` repository.
- Re-run the install or upgrade flow after first-party changes so `~/.codex/` and `~/.agents/skills/` stay in sync.
...
- verify `~/.codex/AGENTS.md` and `~/.agents/skills/` are still aligned with the repository layout
