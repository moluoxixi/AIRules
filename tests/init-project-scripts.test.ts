...
it('init-project inject-rules - AGENTS.md 不存在时创建聚合规则文件', () => withTempDir('airules-inject-create-', (tmpDir) => {
  ...
  assert.match(agentsContent, /## 测试文档结构/)
  assert.match(agentsContent, /docs\/test\/e2e/)
  assert.doesNotMatch(agentsContent, /# 子代理委派/)
  assert.doesNotMatch(agentsContent, /关键环节子代理调度索引/)
  assert.doesNotMatch(agentsContent, /# 变更分级与确认门禁/)
  assert.doesNotMatch(agentsContent, /# 变更分级与澄清门禁/)
  assert.doesNotMatch(agentsContent, /\bcontrol\.md\b/)
  assert.doesNotMatch(agentsContent, /\bsubagent\.md\b/)
  assert.doesNotMatch(agentsContent, /规则自足性校验/)
  assert.doesNotMatch(agentsContent, /rules\/sources/)
  assert.doesNotMatch(agentsContent, /init-project reference/)
...
it('init-project inject-rules - AGENTS.md 已存在且无重复标题时追加规则', () => withTempDir('airules-inject-append-', (tmpDir) => {
  ...
  assert.doesNotMatch(agentsContent, /\bsubagent\.md\b/)
  assert.doesNotMatch(agentsContent, /规则自足性校验/)
  assert.doesNotMatch(agentsContent, /rules\/AGENTS/)
  assert.doesNotMatch(agentsContent, /init-project reference/)
