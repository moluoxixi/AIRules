...
it('前端编码规范 - SKILL.md 是唯一规则源', () => {
  ...
  assert.match(skill, /重点约束目录拆分、入口模型、模块边界、公共导出和 import 路径/)
  assert.match(skill, /业务实现是否抽离由项目既有模式和真实维护成本决定/)
  assert.match(skill, /不得仅因规则存在而强制拆分/)
  assert.doesNotMatch(skill, /状态、跨组件复用逻辑和副作用是否抽离/)
  ...
  assert.doesNotMatch(skill, /局部状态、跨组件逻辑或副作用/)
  ...
  assert.match(skill, /注释说明 Why over What/)
  ...
  assert.match(skill, /类型边界：复杂组件如需拆分 Props、Emits、Expose、Ref/)
...
it('前端编码规范 - 类型组织示例保持建议口径', () => {
  ...
  assert.match(skill, /类型推导优先/)
  assert.match(skill, /现有组件、API 响应、Schema 或常量对象表达的类型/)
  assert.match(skill, /避免重复手写/)
  assert.match(skill, /类型边界：复杂组件如需拆分 Props、Emits、Expose、Ref/)
...
it('前端编码规范 - 注释规范保持建议口径且不强制剥离副作用', () => {
  ...
  assert.doesNotMatch(skill, /副作用/)
  assert.doesNotMatch(skill, /复杂副作用必须/)
  assert.match(checklist, /目录拆分是否服务真实边界/)
  assert.match(checklist, /没有为形式化拆分创建空目录或装饰性层级/)
  assert.doesNotMatch(checklist, /复杂副作用和核心契约是否说明设计意图/)
})
