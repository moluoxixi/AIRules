...
it('node 编码规范 - 入口引用 Node 后端实现标准', () => {
  ...
  assert.match(skill, /SIGINT\/SIGTERM/)
  assert.match(skill, /Graceful Shutdown/)
  assert.match(skill, /防阻塞主线程/)
  assert.match(skill, /worker_threads/)
  assert.match(skill, /AsyncLocalStorage/)
  assert.match(skill, /Awilix/)
  assert.match(skill, /uncaughtException/)
  assert.match(skill, /unhandledRejection/)
...
it('node 编码规范 - skill 自身承载示例与清单', () => {
  ...
  assert.match(skill, /FastifyInstance/)
  assert.match(skill, /zod/)
  assert.match(skill, /return reply\.code\(201\)\.send\(orderPresenter\.toResponse\(result\)\)/)
  assert.match(skill, /Service 只做用例编排与事务边界/)
  assert.match(skill, /repository 负责持久化访问和映射/)
