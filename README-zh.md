...
- 标准 `.agents/skills`、`.claude/agents` 与根 `AGENTS.md` 会映射为角色的标准 skills、agents 与 rules；完整 role path 同时保留精选的多宿主原生适配。AIRules 自有的 `init-project` skill 使用角色内迁移资产生成项目 runtime 与 18 个平台适配，不安装或调用 Trellis npm CLI。
