...
describe('编排红线文本', () => {
  ...
  it('ecc-development role 接入 ECC 并分发 fallback baseline', async () => {
    ...
    assert.match(roleReadme, /claude-project/)
    assert.match(roleReadme, /Codex.*没有 `codex-project`/s)
    assert.match(roleReadme, /Cursor.*项目级/s)
    assert.match(roleReadme, /Qoder.*不支持/s)
    assert.match(roleReadme, /Trae.*不支持/s)
    assert.match(roleReadme, /Trae Solo.*只有 MCP 面/s)
    assert.match(roleReadme, /Hermes.*缺少已审计 ECC MCP\/hook 对齐面/s)
    assert.match(roleReadme, /CC-Switch \/ QoderWork.*不启用 fallback/s)
    assert.match(roleReadme, /ECC core 安装面/)
    assert.match(roleReadme, /Claude \/ Codex 差异根因/)
    assert.match(roleReadme, /Claude core 选中 `rules-core`/)
    assert.match(roleReadme, /Codex core 只选中 `agents-core`/)
    assert.match(roleReadme, /Agents 数量差异不是格式转换的结果/)
    assert.match(roleReadme, /Codex target 额外携带的 Codex 原生 TOML agents/)
    assert.match(roleReadme, /fallback 支持判定/)
    assert.match(roleReadme, /三宿主 fallback 改造/)
    assert.match(roleReadme, /能力裁剪后的 fallback/)
    assert.match(roleReadme, /只有 MCP 目录存在时必须跳过/)
    assert.match(roleReadme, /SharedClientCache/)
    assert.match(roleReadme, /AppData\/Roaming\/Trae CN\/User\/mcp\.json/)
    assert.match(roleReadme, /不承接的官方模块/)
    assert.match(roleReadme, /不承接 `hooks-runtime`/)
    assert.match(roleReadme, /skills.*Markdown agents.*MCP/s)
    assert.match(roleReadme, /ECC 专属 rules \/ hooks 不作为 fallback 分发面/)
    assert.match(roleReadme, /全局规则边界/)
    assert.match(roleReadme, /rules\/AGENTS\.md` 分发中性 fallback baseline/)
    ...
    assert.match(roleReadme, /commands 分发通道/)
    assert.match(roleReadme, /--with lang:\*/)
    assert.match(roleReadme, /--with framework:\*/)
    assert.match(roleReadme, /Qoder.*AIRules/s)
    assert.match(roleRules, /^# ECC Fallback Baseline/m)
    assert.match(roleRules, /适用于通过 AIRules fallback 投影承接 ECC core 能力的宿主/)
    assert.match(roleRules, /skills\/`/)
    assert.match(roleRules, /agents\/`/)
    assert.match(roleRules, /不要把本 baseline 解读为 Claude `rules-core`/)
    assert.match(roleRules, /联网、发布、推送、合并、修改第三方资源或凭据前必须取得用户明确授权/)
    assert.match(roleRules, /优先遵循当前项目的 `AGENTS\.md`/)
    assert.match(roleRules, /缺失必需信息时显式失败/)
    assert.doesNotMatch(roleRules, /# ECC for Codex CLI/)
    assert.doesNotMatch(roleRules, /\.codex\/config\.toml/)
    assert.doesNotMatch(roleRules, /codex mcp add/)
    assert.doesNotMatch(roleRules, /features\.multi_agent/)
    assert.doesNotMatch(roleRules, /Use `\/agent`/)
    assert.doesNotMatch(roleRules, /sandbox_mode = "workspace-write"/)
    assert.doesNotMatch(roleRules, /Hooks \| 8\+ event types/)
    assert.doesNotMatch(roleRules, /Context file \| CLAUDE\.md \+ AGENTS\.md/)
    assert.doesNotMatch(roleRules, /Skills are auto-loaded from `\.agents\/skills\/`/)
    assert.doesNotMatch(roleRules, /AGENTS\.md only/)
    assert.doesNotMatch(roleRules, /Not yet supported/)
    assert.doesNotMatch(roleRules, /npx -y --package ecc-universal/)
    assert.doesNotMatch(roleRules, /official ECC install/)
    assert.doesNotMatch(roleRules, /global install/)
    assert.doesNotMatch(roleRules, /project-level target/)
    assert.doesNotMatch(roleRules, /role sync/)
    assert.doesNotMatch(roleRules, /--profile/)
    assert.doesNotMatch(roleRules, /--target/)
    assert.doesNotMatch(roleRules, /fallback is enabled only/)
    assert.doesNotMatch(roleRules, /MCP-only projection/)
    assert.doesNotMatch(roleRules, /roles\/ecc-development\/mcp\/mcp\.json/)
    assert.doesNotMatch(roleRules, /Do not run ECC fallback/)
    assert.doesNotMatch(roleRules, /online upstream skills, online upstream agents/)
    assert.doesNotMatch(roleRules, /full upstream MCP catalog/)
    assert.doesNotMatch(roleRules, /auto-enable ECC hook behavior/)
    assert.doesNotMatch(roleRules, /--with lang:\*/)
    assert.doesNotMatch(roleRules, /--with framework:\*/)
