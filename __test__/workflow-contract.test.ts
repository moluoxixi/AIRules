...
describe('编排红线文本', () => {
  ...
  it('ecc-development role 接入 ECC 并保留为空第一方 role', async () => {
    ...
    assert.equal(roleRules, '')
    assert.doesNotMatch(roleRules, /\bECC\b/i)
    assert.doesNotMatch(roleRules, /fallback/i)
    assert.doesNotMatch(roleRules, /install/i)
    assert.doesNotMatch(roleRules, /target/i)
    assert.doesNotMatch(roleRules, /host/i)
    assert.doesNotMatch(roleRules, /\/project-init/)
    assert.doesNotMatch(roleRules, /official ECC/)
    ...
    assert.doesNotMatch(roleRules, /\bCodex\b/)
    assert.doesNotMatch(roleRules, /\bClaude\b/)
    assert.doesNotMatch(roleRules, /\bQoder\b/)
    assert.doesNotMatch(roleRules, /\bTrae\b/)
    assert.doesNotMatch(roleRules, /fallback is enabled only/)
    assert.doesNotMatch(roleRules, /MCP-only projection/)
    assert.doesNotMatch(roleRules, /rules-core`, `commands-core`, or `hooks-runtime/)
    assert.doesNotMatch(roleRules, /Codex-native TOML agents/)
