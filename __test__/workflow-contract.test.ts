...
const specScriptsDir = path.join(repoRoot, 'roles', 'development', 'skills', 'init-project', 'scripts')
...
describe('自洽检查器', () => {
  ...
  function seedCleanRepo(root: string) {
    fs.mkdirSync(path.join(root, 'roles', 'development', 'agents'), { recursive: true })
    fs.mkdirSync(path.join(root, 'roles', 'development', 'skills', 'writing-plans'), { recursive: true })
    fs.mkdirSync(path.join(root, '.airules', 'knowledge', '架构'), { recursive: true })
    fs.mkdirSync(path.join(root, 'roles', 'development', 'rules'), { recursive: true })
    for (const name of ['coder', 'debugger', 'consistency-reviewer', 'code-reviewer']) {
      fs.writeFileSync(path.join(root, 'roles', 'development', 'agents', `${name}.md`), `---\nname: ${name}\n---\n\n## 加载 skill\n\n- \`writing-plans\`：x\n`)
    }
    // check #11：planner.md 必须包含 .airules/requirements/ 路径声明。
    fs.writeFileSync(
      path.join(root, 'roles', 'development', 'agents', 'planner.md'),
    ...
    for (const skill of firstPartySkillNames()) {
      fs.mkdirSync(path.join(root, 'roles', 'development', 'skills', skill), { recursive: true })
    }
    // check #10：brainstorming/SKILL.md 必须声明需求落档路径。
    fs.writeFileSync(
      path.join(root, 'roles', 'development', 'skills', 'brainstorming', 'SKILL.md'),
      '---\nname: brainstorming\n---\n\n落档路径（强制）：`.airules/requirements/<feature>.md`\n',
    )
    // check #12：writing-plans/SKILL.md 必须引用前后端参考文件。
    fs.writeFileSync(
      path.join(root, 'roles', 'development', 'skills', 'writing-plans', 'SKILL.md'),
    ...
    fs.writeFileSync(path.join(root, 'roles', 'development', 'rules', 'AGENTS.md'), 'Consist -->|符合| Test\n')
  ...
  it('捕获：agent 引用不存在的 skill', () => {
    withTempDir((root) => {
      seedCleanRepo(root)
      fs.writeFileSync(path.join(root, 'roles', 'development', 'agents', 'coder.md'), '---\nname: coder\n---\n\n## 加载 skill\n\n- \`nonexistent-skill\`：x\n')
      const { errors } = checkRulesConsistency(root)
  ...
  it('捕获：rules 含旧时序边 Test -->|PASS| Consist', () => {
    withTempDir((root) => {
      seedCleanRepo(root)
      fs.writeFileSync(path.join(root, 'roles', 'development', 'rules', 'AGENTS.md'), 'Test -->|PASS| Consist\n')
      const { errors } = checkRulesConsistency(root)
  ...
  it('捕获：skills 目录有 SKILL.md 但未登记 constants/skills.ts', () => {
    withTempDir((root) => {
      seedCleanRepo(root)
      const orphan = path.join(root, 'roles', 'development', 'skills', 'unregistered-orphan-skill')
...
describe('编排红线文本', () => {
  it('缺上游事实源 → coder/planner 含 MISSING blocked', () => {
    assert.match(read('roles/development/agents/coder.md'), /MISSING blocked/)
    assert.match(read('roles/development/agents/planner.md'), /MISSING blocked/)
  })

  it('未验证不得 PASS', () => {
    assert.match(read('roles/development/rules/AGENTS.md'), /禁止在未读到实际结果前声称通过/)
  })

  it('reviewer ≠ coder 且不得自评', () => {
    assert.match(read('roles/development/rules/AGENTS.md'), /reviewer ≠ coder/)
    assert.match(read('roles/development/agents/code-reviewer.md'), /不得自评/)
  })

  it('一致性评审时序：编码后、测试验证前（图与文案一致）', () => {
    const rules = read('roles/development/rules/AGENTS.md')
    assert.match(rules, /Consist -->\|符合\| Test/)
    assert.doesNotMatch(rules, /Test -->\|PASS\| Consist/)
    assert.match(read('roles/development/agents/consistency-reviewer.md'), /编码后、测试验证前/)
    assert.match(read('roles/development/skills/consistency-check/SKILL.md'), /编码后、测试验证前/)
  })
  ...
  it('subagent fix→复审循环有界', () => {
    assert.match(read('roles/development/skills/subagent-driven-development/SKILL.md'), /fix→复审循环上限/)
  })
  ...
  it('记忆进化闭环关键文本锚点存在', () => {
    ...
    for (const anchor of ['boundary', 'superseded', '安全边界侵蚀', '库级健康复核']) {
      assert.ok(rules.includes(anchor), `roles/development/rules/AGENTS.md 缺锚点：${anchor}`)
    }
  })

  it('test-design 测试用例必须落档到 .airules/tests/（不只停在对话里）', () => {
    const td = read('roles/development/skills/test-design/SKILL.md')
  ...
  it('planner 同时产出任务文件与测试用例文件', () => {
    const planner = read('roles/development/agents/planner.md')
    assert.match(planner, /\.airules\/tasks\//, 'roles/development/agents/planner.md 缺任务落档规则')
    assert.match(planner, /\.airules\/tests\//, 'roles/development/agents/planner.md 缺测试用例落档规则')
    assert.match(planner, /测试用例文件路径/, 'roles/development/agents/planner.md 回传字段缺"测试用例文件路径"')
  })

  it('brainstorming 声明需求落档路径 .airules/requirements/', () => {
    assert.match(
      read('roles/development/skills/brainstorming/SKILL.md'),
  ...
  it('planner 接收需求文档路径（.airules/requirements/）', () => {
    assert.match(
      read('roles/development/agents/planner.md'),
      /\.airules\/requirements\//,
      'roles/development/agents/planner.md 输入上下文包未包含需求文档路径 .airules/requirements/（下游追溯链断裂）',
    )
  })

  it('writing-plans/SKILL.md 引用前后端计划参考文件', () => {
    const wp = read('roles/development/skills/writing-plans/SKILL.md')
  ...
  it('coder 输入包含测试用例文件（.airules/tests/）', () => {
    const coder = read('roles/development/agents/coder.md')
    assert.match(coder, /\.airules\/tests\//, 'roles/development/agents/coder.md 输入上下文包缺 .airules/tests/ 引用')
  })

  it('consistency-reviewer 输入包含 .airules/tests/ 测试用例文件引用', () => {
    const cr = read('roles/development/agents/consistency-reviewer.md')
    assert.match(cr, /\.airules\/tests\//, 'roles/development/agents/consistency-reviewer.md 缺 .airules/tests/ 引用')
  })

  it('subagent-driven-development 任务派发单位包含测试用例文件引用', () => {
    const sdd = read('roles/development/skills/subagent-driven-development/SKILL.md')
...
describe('回路熔断承载与 blocked 消费契约', () => {
  ...
  it('rules blocked_id 定义含消费契约：产出方 + 消费方（O-02）', () => {
    assert.match(read('roles/development/rules/AGENTS.md'), /消费契约/)
  })
  ...
  it('5 个 agent 输入上下文包均含"读账本→回执 BLOCKED"消费契约（O-02）', () => {
    for (const a of agentFiles) {
      assert.match(read(`roles/development/agents/${a}.md`), /MUST 读进度账本/, `roles/development/agents/${a}.md 缺读账本消费契约`)
      assert.match(read(`roles/development/agents/${a}.md`), /affected_downstream/, `roles/development/agents/${a}.md 缺 affected_downstream 判定`)
    }
  })
  ...
  it('三个出结论 agent 输出契约含 current_loop_id / current_iteration / recommended_next_action（O-03）', () => {
    for (const a of ['consistency-reviewer', 'code-reviewer', 'debugger']) {
      const content = read(`roles/development/agents/${a}.md`)
      assert.match(content, /current_loop_id/, `roles/development/agents/${a}.md 缺 current_loop_id`)
      assert.match(content, /current_iteration/, `roles/development/agents/${a}.md 缺 current_iteration`)
      assert.match(content, /recommended_next_action/, `roles/development/agents/${a}.md 缺 recommended_next_action`)
    }
  })

  it('code-reviewer 输出 should_increment_mismatch_loop 字段（O-03）', () => {
    assert.match(read('roles/development/agents/code-reviewer.md'), /should_increment_mismatch_loop/)
  })
  ...
  it('check #9 能捕获种入宿主目录引用的 skill（E-01）', () => {
    withTempDir((root) => {
      const badSkill = path.join(root, 'roles', 'development', 'skills', 'rogue-skill')
  ...
  it('rules scope 判定段含宿主目录文本锚点（E-01）', () => {
    assert.match(read('roles/development/rules/AGENTS.md'), /项目级 skill 不得在安装脚本或 SKILL\.md 中引用宿主全局目录/)
  })
